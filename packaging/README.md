...
secret (a PAT with `public_repo` and `workflow` scope). It fast-forwards the token
account's fork first, because a fork that trails upstream cannot open a pull request.
`workflow` scope is what that sync needs: upstream edits its own workflow files, and
GitHub refuses a write touching `.github/workflows/` without it. Without the secret it
skips, green.
