...
Non-breaking by construction: a new table, nothing altered. Every snapshot taken before this
simply has no rows, and the simulator reads that as "this scan did not record it" and refuses
the season card, never as an empty bundle to replay a plan from -- the whole point being that
a plausible wrong number on the screen where someone chooses a deletion threshold is worse
than a blank. It heals on the next scan. Testers never rebuild their database.
