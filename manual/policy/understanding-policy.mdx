...
description: "Your policy is the rulebook Reaper follows when it decides what to remove. Here is how it thinks, and how to set it up safely the first time."
sidebar_position: 2
---
...
Above all, leave the time-to-be-rewatched line at its 3-year default. This is the single most important line in the whole tool, and it is a hard line so nothing can outvote it.
...
Open several flagged titles and read why each was flagged, and which protections were checked and did not fire. Notice how much of the pressure is just dormancy.

</Step>

<Step title={"Tune with the simulator, one nudge at a time."}>

Move the line down a step and watch the count, the space reclaimed, and the named titles change live. Change one control, look at the number, repeat. Tighten gradually over several scans.

</Step>

<Step title={"Set pace and grace, then arm deletion last."}>

Confirm your caps and grace, and turn on the Leaving Soon shelf so your household can rescue anything about to go. To see the shelf before you arm anything, also turn on "Update while read-only" in Settings, Plex. Only then arm deletion, which is password-gated and separate from all tuning.
...
| --- | --- | --- | --- |
| How long it's gone unwatched | How long it has sat untouched, counted from the last play or from the day it arrived | 70 | 60 |
...
Starting points for common goals. All of them are grounded in the shipped defaults.
...
A limit on how much one run, or a rolling 30 days, may remove, while "Limit how much each run removes" is on. It stops the run rather than trimming it.
