...
Scores are based on how long a title has sat untouched. Your history source tells Reaper when a title was last played and how far back it can look. Connect it and let one scan finish against real data before you tune anything.
...
A signal is one reason to remove, and it adds up to its number of points to the score.

</Def>

<Def term={"Protection"}>

A hard line keeps a title regardless of its score and can never remove one.

</Def>

<Def term={"Grace"}>

The grace window is the number of days a flagged title shows as leaving so someone can watch it or you can spare it. You still start every removal by hand.

</Def>

<Def term={"Cap"}>

If you turn on "Limit how much each run removes", you can set a limit for a single run or a rolling 30 days. The run stops once it crosses that limit.
