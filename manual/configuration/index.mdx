...
variable can be removed. The import runs on *every* start, not only the first, so you can add a
service to the environment later and it is picked up on the next restart.
...
Plex is not configured here. It is added with **Sign in with Plex** in the setup wizard, which
discovers your server and its token for you.
