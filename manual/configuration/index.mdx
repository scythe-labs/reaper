...
Two homes. **`.env` holds only what must exist *before* the database,
...
The label between the service and the field, `HD` above, is yours to choose. Its case is ignored,
so `hd` and `HD` are one service, not two.
