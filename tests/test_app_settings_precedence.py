...
every mutation of it widens what the app may do: a deletion switch turned off in the UI re-arms
itself on the next restart, and a proxy list the operator cleared goes on deciding auth.
"""
...
#: Two distinct webhook URLs, so a case cannot pass by reading the wrong one (rule 141). The
#: host is Discord's because ``api/settings.py`` validates it there; nothing here posts.
...
def _seeded(tmp_path: Path, **overrides: object) -> Settings:
    """A ``Settings`` carrying one seed, always a value the shipped default is not (rule 141):
    every switch below ships off, every string ships empty, so an omitted seed and a correct
...
class TestTheStoredValueWinsOverTheEnvironmentSeed:
    ...
    async def test_an_emptied_proxy_list_is_kept_rather_than_reverting_to_the_seed(
        ...
        """Rule 1: an explicit empty list is a choice, never "nothing stored". Both states are
        reachable, because ``GeneralSettingsIn.trusted_proxies`` defaults to ``None`` and stores
        ``[]`` as itself, so an operator who clears the list while trust stays on has said to
        trust nobody. Falling back to ``REAPER_TRUSTED_PROXIES`` there leaves a forwarded header
        from a proxy they removed still deciding auth (rule 101).
...
def _env_seeded_getters() -> set[str]:
    ...
    Read off the AST rather than the source text (rule 147): a call to ``_get`` by name, and a
