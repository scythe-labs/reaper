...
Both answer "would this policy delete this item" against the same snapshot. The scan
answers with the full evidence in hand. The simulator answers later, from nothing but the
stored row. If the two answers can differ, the review queue can show an item as "not judged"
while the policy editor counts it as one of the deletions.

Everything must decide on the same stored integer score. A score of 69.7 against a
threshold of 70 abstains as a float, but rounds to 70 when stored. If the scan compared the
float and the simulator later compared the stored 70, the simulator would condemn a film the
scan itself abstained on. So both sides round once, store that, and decide on the stored
integer.
"""
...
def _hand_reap(evaluation: Evaluation, score: int, policy: PolicyBody) -> str:
    ...
    operator presses Reap, whether in the queue, in the plan, or in the executor's per-item
    re-read. The frozen document is produced here by the real writer (``snapshot._explain``)
    rather than hand-typed, so a field the writer stops emitting fails these tests instead of
    quietly changing what the read side can see.
    """
...
def _simulator_verdict(score: int, coverage_bp: int, condemn_at: int, floor_bp: int) -> str:
    ...
    actually makes it, using the real shared function with the stored integers.

    The route imports ``decide_verdict`` and calls it exactly like this for every row it
    re-decides. Protected, blocked, and overridden rows are never re-decided, which is
    pinned in ``TestRowsTheSimulatorMustNotReDecide`` below and in the route-level tests in
    ``test_api.py``. So a regression in the shared function fails here, and a route that
    stops using it fails the route-level boundary test.
    """
...
class TestAReapOverrideForcesCondemnButNeverPastSafety:
    ...
    It beats every cautious protection, whether fired or merely unverifiable, and stops only
    ...
    A blocked gate means Reaper could not answer a question. The owner, standing at the panel
    that names the failed check, can answer it, so an unverifiable protection must not hold a
    hand reap either. Streaming safety here does not rest on the scan-time read: a live read
    at send time, ``executor._being_watched_now``, re-polls Plex per item and spares on any
    failure to read, pinned by
    ``test_reap_loop.py::TestStreamingVeto::test_plex_unreadable_fails_closed``, alongside
    ...
    def test_a_reap_override_does_not_delete_an_unmanaged_file(self) -> None:
        """No *arr owns it, so there is no path to delete through, and reaping it would be
        a lie.

        The detail here matches no live producer, because the gate itself is retired as
        unreachable. ``GateId.UNMANAGED`` survives only so an older stored explanation still
        decodes (``engine.gates``, below ``DataHorizonGate``). This row is that older shape,
        and its half of ``STRUCTURAL_GATES`` is kept for it, so the string stays hand-written
        on purpose.
        """
    ...
    def test_a_reap_override_passes_a_streaming_check_that_could_not_run(self) -> None:
        """The gate where a hand reap looks least safe, and the reason it still is safe.

        A blocked streaming gate means "we could not read who is playing", frozen minutes or
        hours before anything is sent. It does not hold the reap, because the real check now
        happens at send time. ``executor._being_watched_now`` re-polls Plex for this exact
        item and returns "watched" on any read failure, so the live veto supersedes the
        scan-time guess and fails closed where the scan could only guess. A fired streaming
        gate is a different claim and still holds the reap, as pinned above.
    ...
    def test_a_reap_override_condemns_past_any_gate_that_could_not_be_checked(
        ...
        """Swept across four distinct gates, not one, because a single case cannot tell "no
        gate holds" from "this one gate happens to be permitted". ``min_dormancy`` is in the
        sweep deliberately: ``engine.gates`` calls it the most important gate, so if any
        block were still to hold, a reader would expect it to be this one.

        The paired ABSTAIN result shows this is not a blanket loosening. A block still keeps
        the item out of every automatic path. All that changed is that the owner may answer
        it."""
        ...
        # history"), so no single real sentence fits all of them. What is asserted is the
        # verdict on either side of the override, not the wording.
        blocked = Evaluation(
    ...
    #: ``engine.verdict``'s docstring rather than derived from either implementation. Used
    #: by the agreement sweep below.
    ...
    def test_the_stored_row_reaps_each_shape_the_way_the_spec_says(
        ...
        off nothing but the frozen explanation, and serves three surfaces from there: the
        queue's Reap button, the plan, and the executor's per-item re-read. Nothing else
        decides a reap, since the scan's ``_verdict`` takes no override, so there is no
        second implementation to hold this in step with. This sweep is the whole of the reap
        branch's coverage, not half of an agreement pair.
        ...
        written out in ``_REAP_SHAPES`` from the decision's spec, so a shape the reader gets
        wrong still fails even though only one implementation is left."""
    ...
    def test_a_reap_override_beats_a_soft_protection_like_a_rating_floor(self) -> None:
        """A rating floor is a cautious keep, not a safety guarantee, so the owner may
    ...
    def test_a_reap_override_overrules_a_keep_rule_conflict(self) -> None:
        """The keep-rule conflict flags a season for a human to decide. It is a blocked
        ABSTAIN that says "you decide" rather than "could not check", marked by the producer
        with ``defers_to_owner``. With no override it abstains, needing a look. A hand reap
        is the decision it asked for, so it condemns, unlike a protection that could not be
        checked, which still holds."""
        conflict = Evaluation(
            results=[
                GateResult(
                    GateId.SEASON_PROGRESSION,
    ...
    def test_the_wording_of_a_season_block_decides_nothing_either_way(self) -> None:
        ...
        Neither a conflict whose comparison was refused, nor one where the comparison was
        made, may hold or release the reap based on wording such as
        ``detail.startswith("could not check")``, since a real message can open with the
        watcher count instead and never match that prefix at all. This test asserts that the
        wording decides nothing.
        ...
        not pick the verdict. Each shape also abstains with no override, so the item still
