...
"""The shared identity resolver. This is the one place an *arr item binds to its Plex row.

Every case here is a way a wrong bind could delete the right file for the wrong reasons,
such as reading a stranger's watch history. Data throughout is synthetic: generic titles,
and ids shaped like ``tt000000N`` / tmdb ``100N`` / tvdb ``200N``, never a real title.
"""
...
class TestTheIdTierBindsAndDisambiguates:
    ...
    def test_a_duplicate_id_abstains_and_does_not_fall_through(self) -> None:
        """Two Plex items share one tmdb. Ambiguous -> abstain. It must not fall through to
        a title+year match that would 'resolve' it by guessing."""
...
# ---------------------------------------------------------------------------
# Narrowing an ambiguous id by the *arr's own file name. This corroborates inside the
# id's candidate set, and never consults the wider library.
# ---------------------------------------------------------------------------
...
class TestAnAmbiguousIdNarrowedByFileName:
    ...
    *arr item's file name may pick which copy this entry manages. Anything less than
    ...
    def _split_library(self) -> PlexIndex:
        # The same movie sits in two sections. The file names carry the quality marker.
    ...
    def test_a_file_name_matching_both_abstains_without_sizes(self) -> None:
        """The curated-section case. The same physical file is re-listed, so both copies
    ...
    def test_a_copy_with_unknown_files_abstains(self) -> None:
        """One candidate's files could not be seen. That copy might be the very file this
        item manages, so "could not look" never counts as "looked and it was different",
        even though the other copy's name matches."""
        index = PlexIndex.build(
            [
                _item(100, year=2020, tmdb=1001, basename="example (2020) 1080p.mkv"),
    ...
    def test_a_failed_narrow_never_falls_through_to_the_wider_library(self) -> None:
        ...
        map, and title+year would uniquely resolve that same third item. Falling through
        would bind outside the id's candidate set. That is a guess, so this abstains."""
        index = PlexIndex.build(
            [
...
class TestByteIdenticalTwinListings:
    ...
    that size are byte-identical twins of the *arr's own file, since a curated section can
    re-list the very file under its own rating key at another path. They bind as one
    group, with every listing's key kept so watch reads cover them all. Any unknown on
    ...
    def _relisted_library_with_paths(self) -> PlexIndex:
        # The same one-file-listed-twice shape, with the paths a live check found. The
    ...
    def test_twin_listings_bind_as_a_merged_group(self) -> None:
        ...
        assert res.status is MatchStatus.MATCHED
        assert res.matched_by is MatchedBy.MERGED_LISTINGS
        # Canonical is the earliest listing. The group carries every listing's key.
        assert res.rating_key == 100
        assert res.merged_rating_keys == (100, 200)
    ...
    def test_a_size_singling_out_one_listing_binds_it(self) -> None:
        ...
        upgrade). The *arr's exact size picks its own file's listing, with no merge."""
        index = PlexIndex.build(
            [
    ...
    def test_an_unknown_listing_size_keeps_abstaining(self) -> None:
        """One matching listing's size could not be seen. It might be the same file, or it
    ...
    def test_two_twins_merge_while_the_odd_size_stays_out(self) -> None:
        """Three same-name listings: two byte-identical twins and one at another size. The
        twins merge. The odd listing never enters the group."""
        index = PlexIndex.build(
            [
                _item(100, year=2020, tmdb=1001, basename="example.mkv", size=7_000),
    ...
    def test_a_show_never_merges_its_folders(self) -> None:
        """A show is bound by its folder, and a folder has no one size, so two same-name
        folder listings under one id keep abstaining, never merge."""
        index = PlexIndex.build(
            [
                _item(300, title="Example Show", tvdb=2001, basename="example show"),
...
# ---------------------------------------------------------------------------
# The folder corroborator. It is the only thing a show has once the leaf ties.
# ---------------------------------------------------------------------------


class TestTheFolderTellsTwoListingsApart:
    """One title kept in two libraries is listed twice in Plex under one id, with an
    identical leaf name. A folder *above* the leaf can still tell the copies apart, but
    only where it sits strictly below the *arr instance's own root folder, which the
    instance reports and the resolver is handed.
    ...
    This only applies to movies, and that is not an oversight. Radarr writes
    ``<root>/<Title>/<file>``, so a real title folder sits above the leaf. Sonarr writes
    ``<root>/<Show>``. Below a correct root there is only the show folder, which *is* the
    leaf both sides already matched on, so there is no new evidence, and the step stands
    down for every show. See TestAShowNeverGetsFolderEvidence, which pins that.
    """
...
class TestTheFolderNeverBindsOnUncheckableEvidence:
    ...
    def test_a_listing_holding_the_name_twice_is_not_bound_on_its_folder(self) -> None:
        ...
        no single number to compare. The folder points at it, and another copy matches the
...
class TestAShowNeverGetsFolderEvidence:
    """Sonarr writes ``<root>/<Show>``. Below a correct root that is one segment, the show
    folder, which is the very leaf both sides already matched on, so the folder
    corroborator has nothing new to say about a show and always stands down.

    This is worth pinning because the step could only ever fire for a show when its
    failure mode was already present. It could only fire when the path was *deeper* than
    Sonarr's layout, which means the reported root was wrong, and it then bound on
    leftover mount segments. A show with two listings under one id has no size to fall
    back on either, so it abstains, and an abstain keeps the file.
    """
...
class TestTheFolderStepNeverOutranksTheExactSize:
    ...
    Ranking candidates by deepest shared suffix and letting the deepest strictly win is
    unsound here. The shallower copy simply runs out of path, and losing the comparison
    would read as evidence against it when it is nothing of the kind. An exact suffix match
    on the item's library-relative path has no such failure mode, and where the folder
    still points somewhere the exact byte size contradicts, the folder yields. Radarr's
    byte count is exact. A folder name is circumstantial.

    Both shapes below would bind the wrong copy under that ranking, reading a stranger's
    watch history and added-at onto a file somebody watches.
    """
...
# ---------------------------------------------------------------------------
# The contradiction veto. Corroborate or stay silent, never contradict.
# ---------------------------------------------------------------------------


class TestTheContradictionVeto:
    def test_two_id_kinds_pointing_to_different_rows_abstains(self) -> None:
        """Tier 1's own kinds cross-check each other. The tmdb id binds one row, and the
        imdb id names a different one. A mis-tagged external id with no way to know which
        is wrong -> keep."""
        index = PlexIndex.build(
            [
        ...
        # This asserts the *type*, not the sentence. This branch is a disagreement, one row
        # per id kind and different rows, so it must not report as several copies in Plex.
        # An earlier version told the owner exactly that. The keys go with it so the panel
        # can offer a link to each of the two rows.
...
class TestAShowsImdbIdCrossChecksItsTvdbBind:
    ...
    def test_a_stale_tvdb_id_naming_another_show_abstains(self) -> None:
        ...
        assert res.rating_key is None
        assert "contradict" in res.detail.lower()
        # The show ladder reaches the same branch, so it carries the same status and the
        # same candidates.
        assert res.status is MatchStatus.CONFLICTED
        assert res.candidate_rating_keys == (100, 200)
    ...
    def test_imdb_alone_never_binds_a_show(self) -> None:
        """The cross-check may only ever *add* abstains. With no tvdb hit to check, an
        imdb id that names a Plex show does not bind it. That would make shows deletable
        that are kept today. The weaker tiers decide, exactly as before."""
...
class TestTheBasenameCrossChecksAnIdBind:
    """The basename tier runs even when an id already bound, as a cross-check only. It was
    promoted from "consulted only if nothing bound" to always-on, so a corroborating kind
    can raise an abstain it was otherwise structurally unable to raise. The cost is real.
    After an *arr renames a file and before Plex rescans, the candidate's basename and
    Plex's stored one disagree. So disagreement abstains only when the new name positively
    names a *different* row. A name Plex has simply never heard of is silence and leaves
    the id bind standing, or every rename would cost a scan's worth of deletability.
    """
    ...
    def test_a_basename_naming_a_different_row_vetoes_the_id_bind(self) -> None:
        """Positive disagreement, which is the abstain this tier was promoted to catch. The
        id says one row and the file on disk says another. No way to know which is wrong,
        so keep the file."""
        index = PlexIndex.build(
            [
                _item(100, title="Example Movie", tmdb=1001, basename="old name (2020).mkv"),
...
# ---------------------------------------------------------------------------
# Tier 3: title + year. Exactly one implementation is under test here.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# The safety invariant: distinct ids never share a bound key, though the same id may.
# ---------------------------------------------------------------------------
...
class TestTheBindingInvariant:
    def test_two_arr_items_with_different_ids_never_share_a_key(self) -> None:
        ...
        rating key. That is the property that keeps one film from reading another's
        history."""
        index = PlexIndex.build(
            [
    ...
    def test_multi_instance_same_id_shares_one_key(self) -> None:
        """The 4K + HD case. Two arr items (two files) carry the *same* id and both bind
        the one Plex row. This is safe. Deletes route by media_key, and both read the
        same, protective, watch history."""
...
# ---------------------------------------------------------------------------
# The operator's library map. It is ground truth that tells two libraries apart, and the
# only discriminator a show ever gets. Every case is a way a wrong bind could read a
# stranger's watch history, so each ambiguity resolves toward keeping the file.
# ---------------------------------------------------------------------------


class TestCandidateLibrariesForDiagnostics:
    """The raw library titles a duplicated title's copies live in. These are printed in
    the unmatched warning so an operator sees, from the log, whether to map the folder or
    fix its spelling."""
...
class TestTheLibraryMapTellsTwoListingsApart:
    ...
    with an identical show folder. The path cannot split them, but the operator's
    root-folder -> library map can. That map is a declaration, not an inference."""
    ...
    def test_the_stale_mapping_guard_folds_both_sides(self) -> None:
        ...
        listings, and the scan would warn the operator their mapping is wrong."""
    ...
    def test_mapped_library_holds_neither_is_ignored_and_abstains(self) -> None:
        ...
        narrows to nothing, is ignored, and the show abstains as if unmapped. It must
        never cause a silent mis-bind into a library the copy is not in."""
        res = resolve_show(
            ids=ExternalIds.of(tvdb=2001),
            title="Example Show",
            year=None,
...
class TestTheLibraryMapWithMovies:
    ...
    def test_unconfirmed_library_winner_defers_to_size_which_keeps_the_file(self) -> None:
        """The mapped library's copy has no checkable size, while the *other* copy matches
        the arr's byte count exactly. The library step will not bind on the map alone when
        its winner's size cannot be checked and another copy might match exactly. It stands
        down and lets size decide. Size, seeing an unknown size in the set, abstains. So the
...
class TestTheAbstainVocabulary:
    ...
    Multiplicity means several Plex rows answer to this item, so the library really does
    hold more than one copy. Disagreement means each kind of evidence found one row and
    they were different rows, which is Plex and the *arr describing one file differently
    over a library that may hold exactly one copy. Both keep the file, but only one of them
    is true about the number of copies, and every operator surface used to say the same one
    regardless.

    This is written as a table from the four call sites, rather than transcribed off the
    branch structure. The table alone cannot see a fifth call site, though. It drives the
    four it names and passes whatever else exists, so
    ...
    #: A source scan like this one is bounded by the spellings it accepts, so they are
    #: written down here and checked against the tree below.
    ...
    def test_the_table_covers_every_abstain_in_resolve(self) -> None:
        ...
        duplicate that is not there. That is the defect this whole class exists to catch.
        Counting the call sites is what makes the omission visible. A member the table
        never collected is otherwise missing from both the guard and its proof.
        """
        source = (
