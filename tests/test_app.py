...
def test_english_composes_a_nested_reason_param_into_its_own_sentence(client: TestClient) -> None:
    """``refusal.english`` (rule 104's one renderer): a param that is itself a ``Reason`` --
    an ``IntegrationError``/``PlexError`` nested in via ``as_reason()`` -- composes into its
    own catalog sentence rather than the dataclass's ``repr``, the same recursion
    ``why.ts``'s ``composeIn`` does for the identical shape (proven end to end on the
    frontend in ``why.test.ts``). A live route exercises the same path through the HTTP
    layer: ``RefusalHTTPException``/``refusal_body`` render ``detail`` through this
    function too, never a bare ``str.format``, or a nested reason would print as
    ``Reason(id=..., params=...)`` in a real response body.
    """
