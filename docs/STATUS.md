...
| Section nav | **Its own grammar, not the pill track.** A rail whose active cut is a segment of the masthead's own bottom border on a wide screen; under 900px a fixed bottom bar of icons, labels kept in the accessibility tree. The pill track (`.tabs`, `.segmented`) now means only "pick a view of the same set", so navigating and filtering stop looking alike. Reap carries the armed state as a dot, amber when the safety read fails |
