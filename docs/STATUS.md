...
5. **The screen-reader sweep is partly landed.** What landed and why each shape was chosen is
   ...
   Still open: **#173** the backup Restore, the Plex sign-in poll, the simulator and the Plex
   panel; **#177** `aria-busy` unused, a background scan changing every number with no signal, the
