...
| Section nav | **Its own grammar, not the pill track.** A rail whose active cut is a segment of the masthead's own bottom border on a wide screen; under 900px a fixed bottom bar of 24px icons, labels kept in the accessibility tree. The bar is 3.5rem tall because that height *is* the tap target, and it has to clear 44px (WCAG AAA, Apple) and 48px (Material). The pill track (`.tabs`, `.segmented`) now means only "pick a view of the same set", so navigating and filtering stop looking alike. Reap carries the armed state as a dot, amber when the safety read fails. **Settings' own nine-section rail takes the same 900px boundary**: below it the wrapped tabs (two lines, three under 375px) become one `.settings-picker` select, swapped in JS off `NARROW_SCREEN_QUERY` so only one of the two is ever in the tree. The Policy rail keeps its tabs — four labels, and it reports what you have scrolled to |
| Settings saves | **One save bar per panel, the policy editor's `.savebar` reused** (rule 43). General's six per-row Save buttons were rendered inside the right-aligned control box, so the first keystroke shoved the field being typed in 71px sideways. The bar names every unsaved field, sends them in one request, offers Discard, and holds the whole save while the accent hex is half-typed. Controls that take effect the moment they change — the two Switches, the theme and expand-seasons selects — are not drafts and stay out of it |
