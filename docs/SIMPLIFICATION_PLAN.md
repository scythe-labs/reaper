...
- `backup.py`/`restore.py`/`retention.py` — **5 raw `sqlite3.connect` blocks**, none using
  ...
  > rather than reassuring: `arm` writes `READY_MARKER` last and `swap_if_ready` returns on its
