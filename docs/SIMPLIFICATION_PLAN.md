...
| 8 | Dedup and carriers | in progress | 1 of ~25 | W6-2 landed early as #618: #556 made rule 94's bound a live defect, not a tidiness item |
...
- `clients/seerr.py:296-387` — the paging contract written three times; its own test is titled
...
| Rule 56's paging contract | cited by all four loops | `clients/plex.py`'s `_iter_pages` hardened, `history_sync.py:380` with a backstop, `library_index.py:284` with one since #559, `seerr.py:339` and `:364` with **no backstop** | One `paged()` iterator, ~60 lines, risk `safety-path` |
...
> `library_index.py:284` and `seerr.py:339`/`:364`, modeled on `MAX_HISTORY_PAGES`, currently the
