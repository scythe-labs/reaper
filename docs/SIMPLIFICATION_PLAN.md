...
| #699 | 8 | W9-5 and W9-6, both killed | none, prose only | no | **Both wave 9 leftovers killed on the same shape of measurement: the edge each row removes is paid by nobody.** W9-5's correction already said the transitive path survives, and this is that claim as numbers, one module at a time: all 13 that import `IntegrationError` alone reach `clients/base.py` through a client they import for real work, so cutting the direct edge takes **zero** modules off **every** one of their closures. **Three of the row's figures are wrong**: the population is 13 and not 12, 23 modules name the symbol, and the closure is 339 and not 384, of which httpx2 is 24, so "a 384-module httpx closure" misnames what would be saved even if anything were. W9-6 is the opposite measurement reaching the same verdict: the graph **does** move, `api.backup`'s closure going 73 to 38, and that figure has no consumer, because all three importers of `api/backup.py` import `api/runs.py` in the same breath and `reaper.main` loads 928 either way. **All six writers of `app.state.reap_status` are in `api/runs.py`**, so every destination splits the reader off: `api/deps.py` costs no module and is the worst, separating `reap_in_flight` from `_reap_status`, the same `getattr` written to create rather than to refuse; a new `api/reap_state.py` keeps that pair together for +1 on both counters and still leaves the writes behind. **The correction's own "created fresh by `create_app`" is wrong** and is fixed in place: nothing in `main.py` touches `reap_status`, `_reap_status` makes one lazily, and the conclusion survives anyway. **"The only `api → api` edge" is five** (`backup → runs`, `runs:32 → scan`, `plex:54 → settings`, `simulate:30 → policy`, `simulate:31 → review`), the correction having found the second and stopped, and `auth` in the row's exempt set has had no importer under `src/reaper/api/` since W9-2. The layering gate sees none of the five, walking cross-*package* edges only. **Both of W9-6's anchors were stale and are re-anchored**, `:422` to `:472` and `test_scheduler.py:356` to `:357`. No `src/` change, so no counter moves: a kill is what keeps `_EXPECTED_LAYERED_MODULES` at 84 and `_EXPECTED_SOURCE_MODULES` at 116, since building either row would have moved both by one |
