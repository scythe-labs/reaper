...
| #611 | 6 | W2, `components/Settings.tsx` row | `GeneralPanel.tsx`, `JobsPanel.tsx`, `SecurityPanel.tsx`, `NotificationsPanel.tsx`, `ServicesPanel.tsx`, `AboutPanel.tsx`, `BackupPanel.tsx` | no | 2,850 lines out, `Settings.tsx` 3,086 to 236; the seven hold 2,933, so the tree gains 83, the eight headers and import blocks less the seven banners they replace. 2,797 moved lines reconstructed from the base and diffed: **two** differ, the `export` `JobsPanel` and `BackupPanel` need across a file boundary. Shell and `PANELS` byte for byte. **Two pinned per-file populations are the proof no branch changed shape**, and both conserve exactly: 8 query-failure handles to 8 across seven files, 6 reload sentences to 6 across five. The barrel re-exports `isDiscordWebhook` and `MIN_ADMIN_PASSWORD` on `PlexPanel`'s own precedent, so `DiscordModal` and `SetupPasswordStep` are untouched. `JobsPanel` and `BackupPanel` are exported now only because the shell imports them; neither gained a test, so that half of the correction stands. Rule 72 both ways: three moved comments pointed outside their new file, and `index-outside-text.test.ts` asserts a class renders in the file it names, which would have gone red. Seven `refuted.md` rows re-pathed, three of them carrying line numbers already stale at this base. `I18N_PLAN.md`'s "76 files" was **deleted rather than re-derived** — the arithmetic was available and is exactly what #608 got wrong |
...
| `components/Settings.tsx` | 3,086 | 7 panels to their own files; the barrel keeps `PANELS`, the dirty record and the shell | **Landed, and it is 7 panels as the correction says, not 6.** 2,850 lines out, 3,086 to 236; the seven panels hold 2,933 between them, so the tree gains 83 — eight module headers and import blocks, less the seven banner comments they replace. Every moved line is byte-identical bar two: `JobsPanel` and `BackupPanel` take the `export` the shell now needs. The shell and `PANELS` are kept byte for byte. Both pinned per-file populations conserve exactly, 8 query-failure handles to 8 and 6 reload sentences to 6, which is what says no branch moved. `isDiscordWebhook` and `MIN_ADMIN_PASSWORD` re-export from the barrel on `PlexPanel`'s own precedent, so `DiscordModal` and `SetupPasswordStep` are untouched |
...
> `ScheduleModal` moves with `JobsPanel` as one unit. **All of this held on landing.** The two
...
> `api.ts`'s `restoreCancel`), so it needs its own model rather than a shared `OkOut`. `backup.py:326` is a
