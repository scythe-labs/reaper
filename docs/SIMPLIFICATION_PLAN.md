...
> a **+10 net** (+12 added, -2 removed) before the deadline and **+21** with it: bounding a
> fan-out is an addition, and this row was never a duplication to remove.
...
> `engine.url.database` and pins that the sweep vacuums the file the engine opened, which is what
...
| W11-15 | 80 unbounded, `ratelimit.py:200` an unused gate | block above; the gate has 3 callers and sheds load | **built**, +10 net |
