...
**S5 is a line test and has been read as a kill criterion.** "Nets to zero" answers whether an
extraction pays for itself in size. The separate question is **how many places a future author
has to keep in step**. A rule written three times drifts, and CLAUDE.md's rule 144 is the
standing form of it. A helper that saves zero lines and removes a drift surface is worth
building; one that saves zero lines and removes nothing is what S5 is about. A kill needs both
halves said.

**Counted on 2026-08-10.** Four wave-11 kill verdicts state a line figure as the reason:
**W11-8** nets 0, **W11-11** costs +7, **W11-23** nets -7, **W11-44** costs +10. In three of the
four the measured column also carries a premise correction the verdict never mentions, so the
arithmetic is not what those kills rest on. **W11-39 reached its figure the right way round**,
built first and measured at +5. **W3b-9 is rebuilt**, at +13 total lines and -9 code lines,
because the shape that was measured swallowed the `_get` call and the shape that works takes the
value it returns. **W11-41 is the row to copy**: byte-identical DDL means no invariant can drift,
so it names the drift surface as absent rather than counting lines.
...
> as a literal, the last copy of it in the stylesheet. Computed styles diffed either side: five
> lines move and all five are `font-family`. `docs/LEARNINGS.md` carries why five of six correct
> fields reads as correct.
>
...
> `.qty`, `.hex-join` and `.tag-chips` pad nothing and drop out on their own. Count pinned at ten
