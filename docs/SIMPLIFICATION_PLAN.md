...
| 8 | Dedup and carriers | **in progress** | 12 landed, 5 killed | W6-2 landed early as #618: #556 made rule 94's bound a live defect, not a tidiness item. The three already-drifted rows still open are the executor's size interlock (`safety-path`, gated on C9), the two panel heads (a divergence to decide, not inherit), and W3's parameter objects. **C14 is settled** and `_call` is unblocked, the pinning test having landed at #659. W9's `LAUNCHER_CONF_NAME` half landed at #671, the first of its three PRs: measured, 9 cycles to 2, and it moves neither counter S7 said it would. W3's `api/deps.py` is two PRs: the request accessors landed at #670, and the admin-password gate follows as its own `safety-path` PR, gated on C9 |
...
| #PRNUM | 8 | W3, the pragma half of the `backup`/`restore`/`retention` row (killed) | `test_the_journal_mode_pragma_is_set_in_exactly_one_module`, `test_every_prose_copy_of_the_busy_timeout_states_the_declared_value` | no | **The row is killed and the gate lands, and the gate binds a different thing than the row named.** `_configure_sqlite` is already one declaration for both engines; with `_read_revision` and `retention`'s `isolation_level=None` out of scope, the remainder is two one-line `busy_timeout` calls at 5000 and 30000, each value deliberate, plus three sites that need no pragma. **The correction's spread is the right one**: 5000 in one, 30000 in one, absent in three, against the row's 5000 in two and absent in two. The five blocks span four modules and the row's file list misses `db/schema_gate.py`, the module the correction exists to protect. **What is duplicated is the value.** `5000` is a SQL literal in one place and "5s" in five docstrings across `executor`, `imdb_dataset`, `retention`, `scan_runner` and `scheduler`, each citing `db.session` and none derived from it (rule 144); `executor._write_with_retry`'s copy is why the journal write takes two attempts with no sleep between them. The gate reads the seconds out of the declaration, so it cannot drift either, and its set equality catches a reworded copy leaving the walk as well as a sixth arriving. **The scan_runner copy is why the matcher is not a grep**: it says "a write lock it waits only 5s for" and names the pragma nowhere. The second gate holds the correction's parenthetical, `PRAGMA journal_mode=WAL` writing the file it reads, at one module; the three no-pragma sites are the reason, since `schema_gate.stored_revision` reads an operator-supplied backup inside the rule 74 gate and `restore`'s two writers follow it on the same unverified file. Three red demonstrations, each driven: a WAL set added to `restore._force_destructive_off` is named with its line; the declaration moved to 10000 fails all five prose copies by name; one copy reworded to "five-second" fails as a member that left. No `src/` change, so no counter moves |
...
- `backup.py`/`restore.py`/`retention.py` — **5 raw `sqlite3.connect` blocks**, none using
  ...
  > and `scheduler`, each citing `db.session` by name and none derived from it (rule 144).
  > `executor._write_with_retry`'s copy is the reason the journal write takes two attempts with
  > no sleep between them, so moving the pragma makes that reasoning silently wrong on the write
  > that records what was deleted. One of the five carries neither the pragma's name nor a
  > figure a grep for `busy_timeout` finds. So the gate derives the seconds from the declaration
  > and names all five on failure, and a second gate holds the correction's own parenthetical:
...
> `api/auth.py:164-167` before `record_password_failure` is reachable, so all four inherit it
...
| Rule 56's paging contract | cited by all four loops | `clients/plex.py`'s `_iter_pages` hardened, `history_sync.py:380` with a backstop, `library_index.py:284` with one since #559, `seerr.py:352` and `:389` with one since #653 | One `paged()` iterator, ~60 lines, risk `safety-path` |
