...
- `services/plex_link.py:139` — the Plex PIN flow written twice, differing in four tokens. Rules
  ...
  > The contracts differ where W6-3's kill points. `poll_plex_login` mints a session, never stores
  > the token, refuses an account that does not own the already-linked machine, and consumes the
  > pending row on refusal; `poll_link` stores the token, has no ownership check to make because
  > ownership is what it is establishing, and consumes in a `finally`. One function serving both
  > takes a keyword argument, which is the shape that killed W6-3.
  >
