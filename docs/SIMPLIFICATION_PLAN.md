...
| `api/auth.py:230` vs `api/plex.py:129` | `PlexServerChoiceOut` **declared twice under the same class name in two modules**. Pydantic collapses them in `components.schemas` today; the moment either gains a field both operations get module-qualified component names and any generated client breaks silently | 8 | `none` |
...
> verbatim copies, not three — `_refused` (`api/simulate.py`) is the refusal shape. W5-7's line numbers are wrong in the row AND in this
> correction (real: `api/auth.py:230`, `api/plex.py:129`; phase 6 moved both again), and the duplicate **masks the mirror test**:
