...
- **12 modules import `clients/base.py` for `IntegrationError` alone** and pay a 384-module httpx
  ...
  > `sys.modules` at **387**, so a stale 384 total is the likelier reading of the row than a
  > wrong delta. **What is wrong either way is the word httpx.** 145 of the 339 are non-stdlib
