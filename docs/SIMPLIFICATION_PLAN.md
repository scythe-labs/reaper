...
| 8 | Dedup and carriers | **in progress** | 21 findings landed, 12 killed, 4 open, W11 43 unstarted | **The count is findings, and it was PRs until a completeness audit measured it.** 26 sub-PRs closed 21 findings: five closed none (#659 pins what #676 then built, #678 corrects prose, #686 follows up an already-killed row, #683 and #687 land the surviving halves of rows they kill), two closed one each and were never counted (#616, #618, rows written at the audit), and four closed two each (#653, #681, #698, which kills W3b-2 and lands W11-32, and #705, which builds W3b-12 and W9-4). #699 kills W9-5 and W9-6 together, #706 kills W5-1 while landing the pinning test that kill argues for, and #703 kills W3b-9 the same way. **The killed figure can be checked against a table and the landed one cannot.** *Killed while executing* carries one row per finding, 13 of them: these twelve, plus phase 5's W1.1-l. *Landed* is keyed by sub-PR and no row records which phase it served, so the sub-PR figure here is carried forward by hand and the exception list above does not reconcile against it. A findings-per-phase column in that table is what would make it re-derivable. **Open, by ID**: W3b-8 `leaving_soon`'s Plex client, W3b-10′ the operator-string half of the killed pragma row, W3b-11 four frontend hooks, W6-1 the CSS control standard. Four names, four open. **No open item carries `safety-path` any more**: W5-1 and W5-3 were the last two, and each was driven under C9 before it closed. **The list used to carry W3b-4 while the count beside it already excluded that item**, #696 having moved it to phase 9, so the two disagreed by one from the day that PR landed. **W11 is in this phase's scope and has not been started**: it is **44 items over seven lanes**, enumerated `W11-1` to `W11-44` on 2026-08-10 because it had no IDs and the phase could not name what was left. **43 remain**: W11-32 closed at #698, out of the same function W3b-2 was measured in. The original `~25` denominator never contained it. Two of the 44 are defects rather than duplications, W11-15 and W11-40. **What subset lets the wave read `done` is an open scoping call for the owner.** **C14 is settled.** W9 took three PRs, all landed: 9 cycles to 2, and neither counter S7 named moved. W3's `api/deps.py` took two, both landed (#670, #681). **W3b-4 is no longer counted here: it folds into phase 9's W4.1** (owner, 2026-08-10). 22 of its 40 `.set-row` sites are in `GeneralPanel.tsx`, which W4.1 rewrites, and `<SetRow>` and `FIELDS` describe the same rows, so whichever landed second would be written against the other. One PR does both. `:495` stated the collision and never ordered it; there is no order to settle now |
...
| #PENDING | 8 | W3b-8 (killed); W3b-10′ | `test_every_client_carries_the_operators_own_tls_setting`, `_client_construction_sites`, `restore._PREPARE_FAILED`, `TestAPrepareStepThatFails` | no | **Two rows, one kill and one string lift, and both figures were off.** **W3b-8**: six `PlexClient` constructions is right, re-derived by AST here and at the audit's base commit, but they span five modules and only one sits in the file the anchor names. Four of the six spell the construction identically and a pure helper saves five lines at each against twelve to fourteen for the helper, so the net is four to six (S5); it reaches neither `scan_runner` site, both of which build outside the session block immediately before entering the exit stack, so moving construction inside re-opens the leak rule 34 already closed in `leaving_soon`. The row's "reads differently in two" is six: every unlinked branch is that surface's own operator answer. **The row is also right that there is no hole**, `safety` being keyword-only and required and `verify` defaulting to the strict `True`, and that is what shapes the gate: the omission costs agreement, not safety, so an operator with a self-signed certificate would get one surface that cannot reach their server. The gate binds all six plus the fourteen sibling constructions of `TautulliClient`, `SeerrClient` and the two `*arr` (rule 72), twenty pinned, reusing the `*arr` gate's walk rather than a second one. Driven red three ways. `PlexTvClient` is out of the population, with the reason written on the constant: no `verify` parameter, because plex.tv is not an address the operator configured. **W3b-10′**: the pragma row says two blocks share the operator string; there are **four** raise sites in three functions and one is not a sqlite block at all, `_force_recovery_off`'s `OSError` on the staged `launcher.conf`. One declaration now. **The string was half a sentence**: it said what failed and not what it meant for the install, and "Nothing was restored." is checkable rather than reassuring, since `arm` writes READY last and `apply_pending_restore` returns on its absence (rules 21, 126). It matches `api/backup.py`'s existing "That password didn't match. Nothing was restored." **All three arms were unreached by the whole suite** behind 93% module coverage, and no test anywhere asserted the sentence; three do now, each also asserting the staging stays unarmed, uncovered statements 20 to 14. Five mutations driven red, one of them the READY write moved ahead of the prepare steps, which fails all three. The fourth raise site is the `_TABLE_NAME` guard, `pragma: no cover`, named as out of reach rather than covered (rule 147). No behavior change beyond the one string, so `STATUS.md` is untouched |
...
| W3b-8, `leaving_soon`'s Plex client | Six sites is right and they span five modules, only one of them the file the anchor names. Four spell the construction identically, argument for argument by AST, and a pure `(server, box, safety)` helper saves five lines at each against twelve to fourteen for the helper (S5). It reaches neither `scan_runner` site: both build outside the session block immediately before `stack.enter_async_context`, so moving construction inside re-opens the leak rule 34 already closed in `leaving_soon`. The row's own "no hole" holds, `safety` being keyword-only and required and `verify` defaulting to the strict `True`, so what an omission costs is agreement rather than safety. A gate over all six plus their fourteen siblings lands instead | Phase 8, measured before building |
...
- `services/leaving_soon.py:425` — Plex client construction at **6** sites, and the
  ...
  >
  > **The extraction is S5.** Four of the six read the `PlexServer` row and spell the
  > construction identically, argument for argument, compared by AST. A pure
  > `(server, box, safety)` helper saves five lines at each of those four and costs twelve to
  > fourteen with its docstring and imports, so the net is four to six lines. It reaches neither
  > `scan_runner` site: both build the client OUTSIDE the session block, immediately before
  > `stack.enter_async_context`, and moving construction inside would put every other client's
  > construction between the build and the close. That is the leak `leaving_soon._plex_client`'s
  > own comment records having been fixed once already (rule 34).
  >
  > **"Reads differently in two" is six.** Each site's unlinked branch is that surface's
  > operator answer and no two are the same: `None` and then a `PlexError` sentence, a
  > `configured=false` payload, a 400 from `_linked_server`, an empty dict from a best-effort
  > read, and the two scan sites carrying `None` into the sources and the reap gateway. There
  > is no shared branch to unify.
  >
  ...
  > `test_every_client_carries_the_operators_own_tls_setting` binds all six, plus the fourteen
  > sibling constructions of `TautulliClient`, `SeerrClient` and the two `*arr` (rule 72),
  > **twenty in all**. It reuses `test_every_arr_client_is_built_with_the_same_arguments`'s
  > walk rather than adding a second, and inherits that gate's own reasoning: a shared
  > constructor binds only the callers that adopt it. Driven red three ways, `verify` dropped
  > from a Plex site, dropped from `scheduler`'s Tautulli site, and a seventh Plex site added.
  > `PlexTvClient`'s six constructions are out of the population and the constant says why: it
  > declares no `verify` parameter, because plex.tv is not an address the operator configured.
...
- `backup.py`/`restore.py`/`retention.py` — **5 raw `sqlite3.connect` blocks**, none using
  ...
  > "Reaper couldn't prepare this backup. Nothing was restored." The second half is checkable
  > rather than reassuring: `arm` writes `READY_MARKER` last and `apply_pending_restore` returns on its
  > absence, so a raise from any prepare step cannot have restored anything (rules 21 and 126).
  > It is also the shape the same flow already uses one layer up, `api/backup.py`'s "That
  > password didn't match. Nothing was restored."
  >
  ...
  > `_purge_auth_state`'s, and no test anywhere asserted the sentence. Three tests do now, each
  > also asserting the staging is still unarmed, and the module's uncovered statements fall from
  > 20 to 14. Five mutations driven, each red on its own named case: the mapping arm deleted
  > from each of the three, `READY_MARKER` written first (all three red), and one site reworded
  > to a fifth spelling. The fourth raise site is the `_TABLE_NAME` identifier guard, which
  > carries `pragma: no cover` and is named here as out of reach rather than covered (rule 147).
