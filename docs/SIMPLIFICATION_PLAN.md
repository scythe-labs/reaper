...
| --- | --- | --- | --- |
| Rule 40's one control standard | `00-tokens.css:212`, in prose | 10 rule blocks re-declare the same 6 fields; 8 more re-declare the identical focus ring | **Killed, PR-PLACEHOLDER.** The grouped base rule inverts a specificity tie. A gate lands instead, plus the drift it found |
...
>
> **Killed, PR-PLACEHOLDER: the base rule is not safe to hoist, and the gate found the drift it was
