...
| W3b-8, `leaving_soon`'s Plex client | Six constructions is exact, at this tip and at the audit's base, but they span **five modules** rather than the one the anchor names. Four read the `PlexServer` row and pass the same four arguments in the same order, six physical lines collapsing to one, so a helper takes 20 out and costs 14: a net of about six (S5). It reaches neither `scan_runner` site, because both build outside the session block immediately before the close is registered, and moving construction inside re-opens the leak rule 34 already closed at `_plex_client`'s call sites. A gate binding all six plus fifteen siblings to the operator's stored TLS switch lands instead, 21 constructions pinned, reusing the `*arr` gate's walk | Phase 8, PR #720, driven red four ways |
