...
| 10 | Issues that land here | in progress | 3 of 16 | **Claimed 2026-08-11.** **#691 landed and is confirmed, not invalid**: the arm was already reachable from a test that never read the message, so the rig #692 built was not needed. **The count is 16, not 8**: #754 re-measured every open issue against the branch and the set that must be fixed here doubled. Twelve issues were filed after `ISSUE_LANDING_PLAN.md` was written, and the first pass had also lane'd rows on urgency rather than on collision. **#682 is no longer one of them**: it landed at #692 and is recorded as fixed on the branch, which is why the numerator reads 0. The landing plan holds the per-issue reasoning and the order to work them in, and it dies when #552 merges. **The phase-10 entry gate run happened on 2026-08-11 at `a208b4e`** and all ten are green, each alone with its exit code read: `ruff format --check`, `ruff check`, `mypy src/reaper tests/`, `pytest -q -n auto` (4,311 passed, 1 skipped), `npm run build`, `npm run test` (1,405 passed), `npm run lint`, `npm run format:check`, `alembic upgrade head`, `alembic check` |
