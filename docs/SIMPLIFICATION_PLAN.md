...
| #719 | 8 | W3b-11 (killed) | `DiscordModal`, `NotificationsPanel`, `ServicesPanel`, `SetupPasswordStep`, `SecurityPanel`, `test_a_held_test_result_is_stamped_when_its_request_is_issued` | no | **All four extractions killed on S5, and the two defects the duplication was hiding land instead.** The `> Killed:` block sits on the finding body with the arithmetic per sub-item. **Three of the four counts are wrong.** The image-fallback ladder is two ladders at four sites: `Backdrop` and `WhyHero` share one, `ReviewQueue`'s `Poster` and `ScalesPanel`'s share another, and the row's three name neither pairing. The test-result pairing is four sites, not three. The admin-password form is two, which holds, and its drift is three things rather than one. **The dirty-report five is the only figure that holds, and it is the sub-item worth the least**: 20 lines out against 32 back, and what rule 146 asks is per-site, so a hook cannot carry it. All five were re-checked and all five already name their own branches in a comment. **Three of the four test-result surfaces read the fingerprint at settle time**, where the boxes have moved: on the two webhook surfaces the box is never disabled during the send, so pasting a second webhook while the first is being tested leaves "Passed" beside a channel nobody sent to, and the stored `of` matches the live one by construction. All three are on `dev` (`Settings.tsx:1155` and `:2427` before the panel split, `DiscordModal.tsx:63`). **`ServiceModal` had it right and was unpinned**: its three badge tests all edit the boxes with nothing in flight, which reading the fingerprint at either end satisfies. A fourth drives the retype during the request, driven red against the settle-time read. A hygiene gate pins the four surfaces by name and forbids `of:` handed a call outside an `onMutate` line. **Its first draft was fail-open in its own new work**: bounded at the first comma, it read `of: [kind, baseUrl()].join(" ")` as innocent, which the matcher's spellings test caught and nothing in the tree would have. The value is walked to its own bracket depth now. **The wizard's password step announced its live complaint on every keystroke**, `{pw.length} so far` inside a `role="alert"` region on the form that sets the key arming deletion, where `SecurityPanel`'s copy has been `standing` since #394 (rule 72). Its boxes also gain the sibling's `aria-invalid`, and `SetupPasswordStep.test.tsx` drives both arms plus the axe audit the step never had. `_EXPECTED_STANDING` 35 to 36, `_EXPECTED_RENDERING_TEST_FILES` 56 to 57, `_EXPECTED_FRONTEND_MODULES` 204 to 205. **`SecurityPanel`'s placeholder was the one ungenerated copy of the length floor**, `at least 12 characters` as a literal under a declaration whose own comment names the placeholder as derived (rule 7/24, rule 144). No `src/` change, so no counter moves and `STATUS.md` is untouched. Filed #718, the policy editor's two repair-notice renders disagree about `standing` while the comment over one says they do not. On `dev` |
...
| W3b-11, four frontend hooks | All four extractions net positive, and three of the four counts are wrong. The image-fallback ladder is **two** ladders at **four** sites and the row's three name neither pairing: 37 lines out of `Backdrop` and `WhyHero` against 41 back, 33 of them a new leaf module both callers can import without a cycle. The dirty-report **5** is exact and is the sub-item worth least, 20 out against 32 back, because rule 146's obligation is per-site and a hook cannot carry it. The test-result pairing is **four** sites with a union result type at one of them, +23. The password form is **two** with three boxes against two and three complaint branches against two, +15 for 11 lines out. What the measurement found instead is **three of the four test surfaces reading the fingerprint at settle time**, where the boxes have already moved, so a badge vouches for an address nobody tried; plus a live complaint announced on every keystroke on the wizard's password step, and one ungenerated copy of the length floor. All fixed, with a gate over the fingerprint family | Phase 8, PR #719, the gate driven red and its own first draft fail-open |
