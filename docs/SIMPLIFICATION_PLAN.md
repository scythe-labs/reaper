...
> **Landed, and every number in the paragraph above is low.** The dotted citations are **43**
> across 27 files, not "roughly ten": `_chip` alone is cited 15 times, and the correction's own
> revised figure of 39 across 9 symbols counted `src/` while a third of them live in `tests/` and
> `docs/`. Six test files import internals, not five. All 43 re-pathed mechanically off the new
> modules' own symbol tables, so the mapping is the tree's rather than a reading of it. **The
> guard this paragraph says is "worth considering" was written**: nothing else in the repository
> stops the next split leaving 43 stale citations behind, and this one was found only because the
> plan happened to warn about it.
