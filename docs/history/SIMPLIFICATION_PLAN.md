...
| 9 | Declaration tax | **done** | 3 of 3 | **W4.2 landed at #752 and the phase is closed**, 2026-08-11. **Its generator half is refused, not deferred**: the served document carries 1 property description across 749, where the hand-written mirror carries 679 comment lines, so generating deletes documentation that has no source to regenerate from. The four other counts the finding rests on are wrong in the same direction and its block says which. The guard half landed and compares field types over 749 members. **The claim is released.** **C10 settled by assumption, 2026-08-11**, put to the owner as four numbered calls with a recommendation each and answered by nobody, so the recommended option was built and #746's body states it as assumed rather than decided. **W4.1 landed at #746 and the count is 3, not 2: W3b-4 is un-folded.** Phase 8 moved it here because `<SetRow>` and `FIELDS` would describe the same rows; measured against the shape that got built they describe nothing in common, `TEXT_FIELDS` being over draft state and naming no markup, so they are independently landable and `<SetRow>` sweeps all 26 sites in its own PR rather than half-migrating one panel. **W4.1 costs ~174 production lines where the finding predicted ~410 removed**, and its Landed row says which half that is. **Phase 8 recorded no end-of-phase gate run**, which `:172` asks for; it was run on 2026-08-11 at `1e7bd03` and all eight are green, each alone with its exit code read: `ruff check`, `ruff format --check`, `mypy src/reaper tests/`, `pytest -q -n auto` (4,302 passed, 1 skipped), `npm run build`, `npm run test` (1,402 passed), `npm run lint`, `npm run format:check`. **Alembic was not run**, so `alembic upgrade head` and `alembic check` are owed on this tip; phase 9 adds no migration, so they run at the exit rather than here. **The claim was taken at #749 for W3b-4 alone and released here now that it has landed**, 2026-08-11, the second time this phase has been claimed and let go. `in progress` is what stops a second session working it, so it is set before a sweep starts and cleared when the sweep lands, whether or not the phase is finished; `not started` beside a non-zero *Landed* means unclaimed, not that nothing landed, which is how phase 10's row already reads. **Nothing is open.** **W3b-4 landed at #750 and cost ~104 production lines where it predicted ~110 removed.** Its own block and *Landed* row carry the measurement and the `help`-prop reason, which the finding could not have seen. **The end-of-phase gate run was made twice**, because the phase was declared finished at #746 and then re-opened for W3b-4 and W4.2. The run that counts is the one at `4830329`, the tip W4.2 left, on 2026-08-11: all ten green, each alone with its exit code read, the eight of the entry run plus `alembic upgrade head` and `alembic check` (`No new upgrade operations detected`) against the real data dir. Phase 9 added no migration, which is why the two Alembic gates were deferred from the entry run to here. `pytest` is 4,311 passed and 1 skipped, up nine from the entry run's 4,302, and `npm run test` is 1,405, up three from 1,402. The earlier run against the tree #746 left read 4,306 and 1,402 |
| 10 | Issues that land here | **done** | 16 of 16 | **The phase is closed, 2026-08-11.** All sixteen landed as sub-PRs #757, #758, #759, #760, #761, #762, #763, #764, #765, #766, #767, #768, #769, #770 and #771; #736 and #740 shared one (#769), which is why fifteen PRs close sixteen rows. **#691 is confirmed, not invalid**: its arm was already reachable from a test that never read the message, so #692's rig was not needed. **Four rows were fixed differently from what the issue proposed, each said so in its PR**: #558 corrects `env_flag`'s SOURCE rather than promoting four flags to `Settings`, which would turn an unparseable value from a default into a refused boot; #710 answers the ASYMMETRY rather than claiming unbounded growth nobody demonstrated; #740 qualifies its selector rather than folding a tint, since each chip's tone is its own; #726 fixes the class with a `withUnit` helper rather than the one character. **#598's guard tripped on #549's own port**, which is the guard working. **The three issue bodies this branch invalidates are repointed**: #554 to `docs/LEARNINGS.md` with the pre-deletion sha `163b6d8`, #553's two citations to that sha and to `api/plex.py:726`. **PR #560 is closed as superseded**, naming #771. **`docs/ISSUE_LANDING_PLAN.md` records the per-row outcome** and still dies with #552. **The exit gate run happened on 2026-08-11 at the tip #771 left** and all ten are green, each alone with its exit code read: `ruff format --check`, `ruff check`, `mypy src/reaper tests/`, `pytest -q -n auto` (4,360 passed, 1 skipped), `npm run build`, `npm run test` (1,418 passed), `npm run lint`, `npm run format:check`, `alembic upgrade head`, `alembic check`. **The claim is released.** **Nothing is open.** |
...
| C10 | **settled by assumption** | Nobody, 2026-08-11. Put to the owner as four numbered calls with a recommendation each, in its own message, and proceeded on the recommendation rather than blocking. **1**, the table sits at the route over `GeneralSettingsIn`'s fields, not over `app_settings.py`'s 59 functions: 5 of the 9 General-lane settings fit a (key, type, default, env, validator) row and the other 4 each need a different extra field, so a service-level table is the escape hatches the correction warns about. It also leaves `test_app_settings_precedence.py`'s walk intact, which a table replacing `_get` would empty to zero. **2**, `put_general`'s launcher.conf and `os.environ` phase is a declared exception and its two refusals move up into the checking pass, where they were the one arm nothing pinned. **3**, three fields table-driven and three declared exceptions with a written reason each, not six with three hatches. **4**, the accent's default and hex shape are pinned across both languages rather than generated: the stylesheet and `index.html` need a literal to paint before any script runs. Landed at #746 |
...
| #730 | 8 | W6-1 (killed) | `styles-control-standard.test.ts`, `blocksOf`, `--control-pad` | no | **The extraction is killed and the gate lands with the drift it found.** Both stated counts re-derived at the tip and both hold, ten blocks on the standard and eight more carrying the identical focus ring, which is the only pair in this phase to survive re-derivation. **The correction names the wrong hazard**: not `:is()` but a specificity tie. `.set-row .set-control input` is 0,2,1 and beats `.swatch-wrap input[type="color"]` (27-settings-rows.css:218, also 0,2,1) on source order, which is why `:271` re-overrides the pair inside `.hex-join`; hoisting the group to `01-base.css` flips it. Latent, one DOM shape reaching that selector. The extraction nets about -56 lines of 10,614 and leaves `.search-input` and the two font sites as overrides regardless (S5, and the drift argument does not rescue it: the values are already tokens, so what can disagree is PRESENCE, which the gate reads). **The drift is what the measurement found.** `.set-row .set-control input`, `.set-row .set-control select` and `.log-search` set `font-size` and no `font: inherit`, and a UA stylesheet gives a form control its own family, so Chromium paints the Settings control column at 13.76px Arial and the Logs search box at 15px Arial against system-ui at the other eight. On `dev`, fixed here because this row's own gate is what asserts it. `.search-input` held the last literal copy of `--control-pad`, now `padding-left` after the token. **Operator-visible, and the dump bounds it**: computed styles diffed either side over 22 specimens in a real Chromium, 8 elements move and only on `font-family` and `line-height`. `font` is a shorthand, so the inherited 1.55 line height comes with the family and those boxes grow about 6px, which is the standard arriving: in a cluster row the box sat 8.25px shorter than the button beside it and now sits 1.92px off. **The shorthand also took two font details, and the correctness lane caught both**: `.keyfield`'s monospace and `.hexfield`'s tabular figures sit at (0,1,0) under a (0,2,1) `font: inherit`, so a revealed API key came out in the app's sans against its own comment (rule 7/24). Each is re-stated on the rule that already clears the standard for that control's width, one line each. Mocked up first, and re-mocked when the height moved. **The gate needs no exemption list**, a block being on the standard when it pads with the token, which is that token's own definition, so `.qty` and `.hex-join` declare no padding and `.tag-chips` pads with `--space-sm`, all three falling out of the walk on their own. Count pinned at ten against a hand list (145), the unseeable spellings in its header (147), both prose copies named by the failure (144). Seven mutations driven red alone, including one proving the three value assertions go GREEN on an empty walk and only the self-check carries it. **Two first-draft assertions were wrong and only running them found it**: the literal ban matched a substring and failed on `.qty`'s lopsided padding, and the prose check missed a sentence wrapping across a line. `blocksOf` moved to `test/stylesheet.ts` and `styles-scales.test.ts` reads it, one CSS parser rather than two, its four gates re-driven red. `_EXPECTED_FRONTEND_MODULES` 205 to 206 |
...
| #732 | 8 | W11-16, W11-25, W11-26, W11-27, W11-28, W11-37; W11-21 (settled as a kill, its constant landed) | `useScanStatus`, `useGeneralSettings`, `usePolicyProbe.SETTLE_MS`, `styles-hand-fate.test.ts`, `components/ProgressBar.tsx`, `components/BalanceBar.tsx`, `queueIcons.ExternalMark`, `_PAYLOAD_HOOKS` | no | **Six built, one settled against the linter rather than the line count, and two of the seven recorded figures were wrong.** **W11-16 is both keys, and the third staleTime was never a fourth number**: `main.tsx` declares 30s app-wide, so three sites had no override and two had unexplained ones (60s, 300s). Both hooks take the default. **`App.tsx` and `ReapBreakdown` are documented non-callers of `useScanStatus`**, the shell because its 15s idle poll is a different job (it notices a scan nobody on this screen started) and the ledger because it deliberately costs no request of its own. **Closes #712 by making the claim true in one place**: three sites spelled `: false` under a comment saying the poll sits quiet, which is true of the observer and false about the query wherever `Dashboard` is mounted; the hook says so, and names the wizard as the one place it really does stop. **W11-21 settled as a kill.** The three debounces agree on 250 ms and on nothing else: one output versus two on one timer, identity by serialization versus memoization versus a primitive, and one output also set imperatively to skip the wait. The only shape covering all three takes a caller-supplied dependency list, so it carries a permanent `exhaustive-deps` disable at two sites that pass that check today, which is rule 19's drift surface added rather than removed. The number is shared instead, out of the comment that asserted the two files agreed. **W11-25's 12 is right and the strip is not in it**: the score, chip and status families are byte-identical per tone (property order aside), and the strip's four carry a 1.5px dash and a `box-shadow: none`, a fifth set. The group lands in `23-queue-chips.css`, which holds eight of the twelve and loads after the score badge's scan-verdict tones. **Nothing moves, proved twice over 35 class combinations**: jsdom's `getComputedStyle` over the real concatenated stylesheet, and a throwaway text resolver for the `border` shorthand jsdom drops because it carries `var()`. The committed test covers `border` by source order instead, `getComputedStyle` being unable to read it. **The cascade test is scoped to `.strip-sq` because that is the only surface wearing both a verdict class and an override class** (`SeasonStrip`); `handFate` hands the score badge and the chips one class each, so their tones cannot collide. Five mutations driven red alone: hoisting `.strip-ov-reap-refused` above the verdict bases, deleting one `box-shadow: none`, renaming a member out of the walk, splitting a tone into two rules, and moving `.score-*` back above the verdicts. **W11-26 is 3 of 6 and the recorded note is off by one detail**: the two commented copies point at `ScanLine` and at `ScanBar`, not both at `ScanLine`. `ReapBar` is excluded for its own documented reason (the role is on the text, so Children Presentational does not prune Stop) and `ReapConfirm` for its `aria-valuetext`. Rounding `aria-valuenow` at all three is a no-op: `ScanStatus.percent` is an `int` (`api/scan.py:76`). **W11-28's `2 of 6 duplicated paths` is 4, three of them cross-file**, and the third pair (`SelectTick` against the scan toast's check) is two sizes and two stroke widths rather than one glyph, so it is left alone. `ClockGlyph` takes a `size` so the dormancy pill keeps its 12px. **Rule 72 pulled in the other two copies of one aria-label**: `SCANNING_LABEL` was a constant shared by `ScanBar`'s track and its start announcement, and a literal at `ScanLine` and at `SetupScanStep`; it lives in `ScanLine` now. **The queue's own 250 now says at its own site why it is not `SETTLE_MS`**, rather than being vouched for from the other end (rule 144). **Dropping the two `staleTime` overrides is a request-rate change, not only a freshness one**: `refetchOnMount` is left at its default, so the queue re-reads general settings once per 30s of remounting where it was once per 5 minutes. Structural sharing keeps `data` referentially stable, so nothing re-renders and nothing fans out through `QueueSettingsContext`; the request is the whole cost. **One hygiene gate needed a third answer, not a suppression.** `useScanStatus` returns the payload alone, so `status.error` is the scan's own message; the walk already refused to count it through `_QUERY_PRIMITIVES` while the query was inline and destructured, and hoisting it moved the same expression onto an unknown hook bound whole. Filing it as a read adds two handles for branches that do not exist (rule 141), driven red to show it. `_EXPECTED_FRONTEND_MODULES` 205 to 210, then 212 on the merge that brought #728's two in |
...
| #746 | 9 | W4.1 | `_GENERAL_FIELDS`, `TEXT_FIELDS`, `_HEX_COLOR` | no | **The item costs ~174 production lines where the finding predicted ~410 removed**, and correcting that is worth more than repeating it: `api/settings.py` 1,340 to 1,445, `GeneralPanel.tsx` 1,039 to 1,108. `put_general` itself goes 96 lines to 34; the table, four cleaners and two desktop helpers cost more than the if-chain they replace. What it buys is S5's first half. **The two passes stop being a convention**: the checking pass takes the payload and nothing else, so it holds no session to write through, and that is the only half of the promise a test can discriminate -- measured, by moving the desktop checks back below the write loop and watching both route tests stay green, because the commit at the end rolls a half-applied save back either way. **The desktop pair's two refusals move up into that pass**, where they were the one arm nothing pinned. **A coverage gate** reconciles `_GENERAL_FIELDS` plus a written exception list against `GeneralSettingsIn` (rule 103), driven red by adding a field. **The accent is pinned across both languages**: declared once in `app_settings.py` and hand-copied eight times, two of those in TypeScript and two inside a sentence an operator reads, none generatable because the stylesheet and `index.html` need a literal before any script runs; driven red three ways. **The frontend descriptor is the weaker half and its own comment says so**: at three fields the table is longer than the echoes, and the disagreement it prevents has not fired, #90 having been in `default_spare_days`, one of the three deliberately excluded. **No counter moved.** `_MEMBERSHIP_INVENTORY` was expected to break and cannot: it counts `IN`-clause sites and `api/settings.py` has never had one. `_general_out` is **15** lines, not the 66 the handoff carried, so looping it was never the win |
| #750 | 9 | W3b-4 | `SetRow`, `index-outside-text.test.ts`'s `help` entry | no | **26 hand-written settings rows become one component, and the item costs ~104 production lines where the finding predicted ~110 removed.** The three panels grow 34 lines (`GeneralPanel.tsx` 1,124 to 1,141, `PlexPanel.tsx` 1,253 to 1,266, `LogsPanel.tsx` 323 to 327) and `SetRow.tsx` is 70. **The `help` prop is why, and the finding could not have seen it**: a row's help was already wrapped markup, so as an attribute it is either one 300-character line or a `help={<>...</>}` costing a line more than the `<p className="help">` it replaced. Both were built. The string spelling measured 24 lines past 100 columns with a widest of 376, in files that otherwise cap near 100, and was reverted. **So this rests on S5's second half alone**: the three grid areas can no longer be nested wrong, and a `variant` typed wrong fails a test instead of shipping a row at the wrong width. **Every other figure the finding published held**: 13 / 11 / 2 sites, `plain` 10, `cluster` 2, `accent` 1, `dim` 1, `controlClass` 1, `after` 2. **Two checks, both driven red**, in `GeneralPanel.test.tsx` rather than a `SetRow.test.tsx`, because all four class outcomes render on that panel and a synthetic mount would pin the component against itself; `_EXPECTED_RENDERING_TEST_FILES` therefore stays 58 while `_EXPECTED_FRONTEND_MODULES` goes 214 to 215. **The `help` entry in `index-outside-text.test.ts` moved and would have stayed green if it had not**, since `PlexPanel.tsx` still spells `help` outside `.set-rows`, so it was about to vouch for a different element than its selector styles. **`controlClass` is pinned by nothing** and the prop's own comment says so (rule 118): its one caller is a branch `PlexPanel.test.tsx` names as unmounted. **Every one of the 26 sentences was compared against the base, collapsed the way JSX collapses it, and none changed.** No `src/` change, so no counter moves and `STATUS.md` is untouched |
| #752 | 9 | W4.2 (guard half; generator half refused) | `_as_typescript`, `_type_aliases`, `NARROWED`, `WIDENED`, `Explanation.match`'s null | no | **The generator is refused on a measurement and the guard lands instead.** The served document holds 1 property description across 749 where `api.ts` holds 679 comment lines, so generating deletes documentation with no source to regenerate it from; the finding's block above carries the other four counts it got wrong. The guard now compares field TYPES over 749 members of 92 paired types, and found three typed non-null against a server that sends `null`, all three already guarded at every call site. Deliberate differences are listed by name in `NARROWED` (25) and `WIDENED` (7), not counted, so a swap fails (rule 145). **The item costs +9 production lines where the finding predicted ~1,500 removed**, and the guard goes 840 to 1,339. Every new assertion was driven red first |
...
| W6-1, one grouped base rule for the control standard | Both counts hold, the only pair in this phase to survive re-derivation. The extraction does not: `.set-row .set-control input` is 0,2,1 and beats `.swatch-wrap input[type="color"]` (27-settings-rows.css:218, also 0,2,1) on source order, so hoisting it to `01-base.css` flips that tie and takes the chrome off a color swatch in a set-control. Latent at this tip, one DOM shape reaching it. Against that the extraction nets about -56 lines of 10,614, leaves three blocks as overrides anyway, and puts a 14-selector list in a file remote from every rule it styles. What the measurement found instead is the drift: two of the ten blocks set `font-size` and no `font: inherit`, so the Settings control column and the Logs search box render in the browser's form font. A gate lands, with that fix | Phase 8, #730, measured before building |
...
covers the 94 `export interface`s only; the 17 `export type` declarations, which is where
...
**Counted on 2026-08-10, over two populations.** Eleven of phase 8's fifteen kills rested on a line
count, eight citing S5 by name, while the same phase found five fresh drifts inside the code those
...
the answer.** Seven of those eleven kills landed one: W3b-8, W3b-9, W3c, W3b-11, W6-1, the pragma row's pair,
and W5-6, whose `NUMBERS` derivation is a rule 103 guard. W5-2 is the eighth. Three landed
...
**S6. `docs/STATUS.md` is nearly full.** 113 of 120 lines and a widest line of 98 of 100,
measured on 2026-08-11; it was full when this was written and closed work has left it since,
enforced at `tests/test_repo_hygiene.py`'s
...
`tests/test_capturable_loggers.py` (**51**), and `_DEFERRED_CROSS_PACKAGE_IMPORTS` (**0** sites,
...
| 9 | Declaration tax | W4.1, W3b-4, W4.2 | `behavior` | Most leverage, widest blast radius, smallest tree. W3b-4 un-folded from W4.1 at #746 |
...
There is no registry. `services/app_settings.py` is **59 hand-written functions over 23 key
constants** (57 when this was written), all wrapping two already-generic helpers, `_get` and `_set`. `put_general` is a
...
> **Landed, 4.1 (#746).** Built as C10's recommended shape, which is narrower than the
> proposal in one way that matters: **the table sits at the route, over `GeneralSettingsIn`'s
> fields, not over `app_settings.py`'s 59 functions.** Measured, 5 of the 9 General-lane
> settings fit a (key, type, default, env, validator) row and the other 4 each need a
> different extra field -- a legacy-boolean thaw, an env string parsed into a stored list, a
> four-level resolve ladder, and a read-only marker for `api_key_set`, which `put_general`
> never writes. A service-level table carrying all four is the escape hatches this wave's
> correction warns about, one layer down. It also keeps
> `tests/test_app_settings_precedence.py`'s walk alive: that gate collects functions calling
> `_get` by name, and a table whose reader holds the only `_get` call empties it to zero,
> which its own module docstring predicted in advance.
>
> **The two-pass shape is structural now, not conventional.** `_cleaned_general_values` and
> `_validated_desktop_values` take the payload and nothing else, so the checking pass has no
> session to write through. That is also the only half a test can discriminate: the commit at
> the end of the route rolls a half-applied save back whichever order the phases run in, and
> both route tests stay green with the desktop checks moved back below the write loop.
> Measured, not argued.
>
> **`put_general`'s third phase is a declared exception, and its two refusals moved up.** They
> fired below the writes, covered only by that rollback, and
> `test_one_bad_field_writes_none_of_the_others` carries no `tray`, so the arm was unpinned. It
> has a test now.
>
> **The figures in the body above are wrong in both directions.** `_general_out` is 15 lines
> and was never a 66-line loop candidate; `put_general` was 96 and is 34; and the item **costs**
> ~174 production lines where the body predicts ~410 removed. #90 was one shared `> 0` condition
> across three echoes plus a fourth that did not handle the field, and the *Landed* row says
> which half of S5 the change rests on.
>
> **W3b-4 is un-folded and lands separately.** Phase 8 moved it here on the premise that
> `<SetRow>` and `FIELDS` describe the same rows. They do not: `TEXT_FIELDS` is over draft
> state and names no markup, `<SetRow>` is over markup and names no field, so either can land
> first. The sweep covers all 26 absorbable sites across `GeneralPanel`, `PlexPanel` and
> `LogsPanel` at once (rule 72) instead of half-migrating one panel here.

> **W3b-4 lives here now, measured and unbuilt (2026-08-11).** #746 un-folded it; what follows
> is the measurement, so the next session builds rather than re-derives.
>
> **28 `.set-row` elements, 26 of them absorbable**, across three files and nothing else:
> `GeneralPanel.tsx` 13, `PlexPanel.tsx` 13, `LogsPanel.tsx` 2. The plan's "22 of 40" holds
> under no single matcher and #744 already said so. The two that are not absorbable are
> `PlexPanel.tsx`'s `set-status` pair, which carry no label, no help and no control.
>
> **The prop surface 26 sites actually need**, which is wider than a triplet and is the honest
> cost: `label`, `help`, `children` (the control), `variant` for the three named stylesheet
> opt-outs (`set-row-plain` 10 sites, `set-row-cluster` 2, `accent-row` 1), `dim` for the one
> ternary site (`GeneralPanel.tsx`'s proxy row), `controlClass` for `PlexPanel`'s
> `set-control server-pick`, and an `after` slot for the two rows carrying siblings BELOW the
> control. `after` has to render inside the row element: `PlexPanel.test.tsx` asserts a
> `<Notice>` is `.closest(".set-row")` of its switch, which is rule 42.
>
> **What it saves and what it does not.** Roughly 4 lines of chrome per row, ~110 across the
> tree; the CSS does not shrink at all, since the classes still have to be emitted. Say both
> halves of S5 in the PR, because the line figure alone does not carry this one.
>
> **Three gates move and one trap.** `_EXPECTED_FRONTEND_MODULES` is 214 and a new
> `SetRow.tsx` makes it 215; `_EXPECTED_RENDERING_TEST_FILES` is 58 and a `SetRow.test.tsx`
> makes it 59, which also has to call `expectNoA11yViolations` or be listed as rendering no
> surface. `index-outside-text.test.ts:158` names `PlexPanel.tsx` by name as the file rendering
> `className="help"`, so moving that class into a shared component fails it until the entry
> moves. The trap is `_FIELD_SM_CONTAINERS`: it pins a per-file, per-tag population of 26
> `.field-sm` boxes over 9 files, and none of the three panels is in it. A `<SetRow>` that
> never spells `field-sm` leaves that gate alone; one that does adds a tenth file and fails it.
>
> **Every control keeps its own `aria-label`.** `.set-label` is a `<span>` and names nothing,
> which two component tests rely on by reaching controls through their labels. A `label` prop
> makes it cheap to forget that, and cheap to put one help paragraph over two controls, which
> rule 45 forbids.

> **Landed, W3b-4 (#750).** All 26 sites are `<SetRow>` now, and the two `set-status` rows stay
> hand-written. **Every count above held**: 13 / 11 / 2 sites, `plain` 10, `cluster` 2, `accent`
> 1, `dim` 1, `controlClass` 1, `after` 2.
>
> **The one figure that did not is the line figure, and it is wrong by 214 lines.** The block
> above predicts ~110 removed. Measured: the three panels **grow** 34 lines and `SetRow.tsx`
> costs 70, so the item spends ~104. The reason is the `help` prop. A row's help paragraph was
> already wrapped markup, and moving it into an attribute makes it either one 300-character
> line or a `help={<>...</>}` that costs a line more than the `<p className="help">` it replaced.
> The first spelling was built, measured at 24 lines past 100 columns and a widest of 376 in
> files that otherwise cap at ~100, and reverted. So the chrome saved per row is real and the
> help paid it back. **S5's second half is the whole of what this rests on**: 26 rows can no
> longer nest the three grid areas wrong, and a `variant` typed wrong now fails a test rather
> than shipping a row at the wrong width.
>
> **The gate predictions were right except one, and the exception is a decision.**
> `_EXPECTED_FRONTEND_MODULES` went 214 to 215. `_EXPECTED_RENDERING_TEST_FILES` is **unmoved**
> at 58: no `SetRow.test.tsx` was written, because nothing on this panel is reachable only
> through a synthetic mount, and the two checks live in `GeneralPanel.test.tsx` where all four
> class outcomes actually render. `index-outside-text.test.ts`'s `help` entry moved to
> `SetRow.tsx`, and it would have stayed green if it had not: `PlexPanel.tsx` still spells
> `help` on a paragraph outside `.set-rows`, so the entry was about to vouch for a different
> element than the one its selector styles. `_FIELD_SM_CONTAINERS` was left alone, as predicted.
>
> **`controlClass` is the one prop no test pins**, and `SetRow.tsx` says so where the prop is
> declared (rule 118). Its only caller is the unlinked `ServerPickList` row, a branch
> `PlexPanel.test.tsx` already names as one its fixture does not mount.
...
> **Landed, W4.2 (#752), and half of it is refused.** The guard upgraded; the generator did not,
> because the premise under it does not hold. The served document carries **1 property
> description across 749**, where `api.ts` carries **679 comment lines** across its 1,235
> declaration lines, and those lines say what no annotation holds: when a field is null and what
> to read instead, which of two spare expiries answers which question, why a three-state field
> must never read as `false`. Generation deletes all 679 with nothing to regenerate them from.
> The counts behind the rest of the sentence are also off. Generation loosens **210** members to
> optional, not "the optional fields the fixtures do not carry": a response model with a default
> drops out of `required` while the route still sends it, so the document's `required` list is
> describing request validation. It loosens **25** to bare `string`, not ~8. It erases six
> `dict[str, X]` to `object`, which the annotations do not. A "small hand-written overlay" would
> have to hold back 239 members, so it is not small and it is not an overlay. The header comment
> in `api.ts` telling a future author to generate these once they grew is corrected in place.
>
> **The guard is the half that was real.** It renders each Pydantic annotation as the TypeScript
> it would have to be written as and compares that against the declaration, over 749 members of
> 92 paired types. **Three members were typed non-null against a server that sends `null`** and
> every call site already guarded them, so the code knew and the declaration did not. The 17
> `export type` aliases join the walk, closing the bound the wave-4 correction named. Optionality
> is still not compared, and that is now a measurement rather than a bound: there is no true
> reference on either side of it. **Cost: +9 production lines, all comment, and the guard goes
> 840 to 1,339.** The finding predicted ~1,500 lines replaced by ~150.
...
| --- | --- | --- | --- |
| Rule 40's one control standard | `00-tokens.css:212`, in prose | 10 rule blocks re-declare the same 6 fields; 8 more re-declare the identical focus ring | **Killed, #730.** The grouped base rule inverts a specificity tie. A gate lands instead, plus the drift it found |
...
>
> **Killed, #730: the base rule is not safe to hoist, and the gate found the drift it was
> meant to prevent.** Both counts re-derived at the tip and both hold, the only pair in this phase
> to survive re-derivation intact: ten blocks pad with `--control-pad`, and beyond the nine focus
> rules paired with them, eight more carry the identical ring. The correction's third deviating
> block is `.search-input`, which it does not name.
>
> **The hazard is not `:is()`.** `.set-row .set-control input` is 0,2,1, and it beats
> `.swatch-wrap input[type="color"]` (27-settings-rows.css:218, also 0,2,1) on source order today,
> which is why `:271` exists to re-override the pair inside `.hex-join`. Hoisting the group to
> `01-base.css` moves it ahead of `:218` and flips that pair, stripping the chrome off a color
> swatch in a set-control. One DOM shape reaches it today (`GeneralPanel.tsx:535`, always inside
> `.hex-join`), so it is latent. **Against that, the extraction nets about -56 lines of 10,614**,
> leaves `.search-input` and two font sites as overrides anyway, and replaces ten local blocks with
> one 14-selector list in `01-base.css` remote from every file it styles. The saving does not pay
> for the tie.
>
> **What the measurement found is the drift.** `.set-row .set-control input, .set-row .set-control
> select` and `.log-search` declare `font-size` and no `font: inherit`, so both render in the
> browser's own form font: Chromium gives 13.76px Arial to the whole Settings control column and
> 15px Arial to the Logs search box, against system-ui at the other eight. On `dev` too, and fixed
> here rather than filed, since the gate is what asserts it. `.search-input` also spelled the pad
> as a literal, the last copy of it in the stylesheet. **Computed styles diffed either side over
> 22 specimens: 8 elements move, on `font-family` and `line-height` and nothing else.** `font` is
> a shorthand, so the inherited 1.55 line height comes with the family and those boxes grow about
> 6px. That is the standard arriving: in a cluster row the box sat 8.25px shorter than the button
> beside it and now sits 1.92px off. **The shorthand also took two font details the correctness
> lane caught**, `.keyfield`'s monospace and `.hexfield`'s tabular figures, both at (0,1,0) under
> a (0,2,1) `font: inherit`; each is re-stated on the rule that already clears the standard for
> its width. `docs/LEARNINGS.md` carries why five of six correct fields reads as correct.
>
> **The gate is `frontend/src/styles-control-standard.test.ts`.** A block is on the standard when
> it pads with `--control-pad`, which is the token's own definition and needs no exemption list:
> `.qty` and `.hex-join` declare no padding and `.tag-chips` pads with `--space-sm`, so all three
> fall out of the walk on their own. Count pinned at ten
> against a hand list (rule 145), what the walk cannot see written into its header (rule 147), and
> both prose copies of the standard named by the failing assertion (rule 144). Seven mutations
> driven red, alone. **Two of its own first-draft assertions were wrong** and only running it
> found them: the literal ban matched a substring and failed on `.qty`'s deliberately lopsided
> padding, and the prose check missed a sentence that wraps across a line. The block walk moved to
> `test/stylesheet.ts` as `blocksOf`, and `styles-scales.test.ts` now reads it, so one parser
> serves both.
...
> narrowed to the two keys that actually diverge. **W11-39** was recommended here at -8 lines, built,
> measured at +5 and killed.
>
...
> render and a timer keyed on its identity restarts forever. `PolicyEditor` sets TWO values off one
> timer, deliberately: the unmeasured allowance rides along so the validator never sees a
> mismatched pair (B-26), and two `useDebounced` calls would be two settles and two requests, the
> first against that pair. `ReviewQueue` sets one value that is ALSO set imperatively, by the jump
> that skips the wait so the operator lands on the filtered list rather than the lane. A
> value-returning `useDebounced(value)` fits none of the three.
>
...
> so the hook carries a permanent, unconditional disable. Two of these three sites pass that check
> today; only `usePolicyProbe` has a disable, justified in place. Rule 19 is about stable effect
> dependencies, so a hook that turns the checker off at two compliant sites adds the drift surface
> the item exists to remove. That is the verifier's own test applied: lines were never the
> question, and this change fails it.
>
> **What did land is the number.** `SETTLE_MS` was declared in `usePolicyProbe.ts` under a comment
> reading "the same 250ms the policy editor debounces its simulate and validate reads on", which is
> one fact asserted by prose about a literal in another file (rule 144). `PolicyEditor` imports it
> now, and the queue's search box states at its own site why its 250 is a separate call.
...
| W11-16 | 4 keys, 4 existing shared hooks, ~40 | block above; 5 declarations each, 3 intervals, 3 staleTimes, the third being the app-wide 30s default | **built**, both keys, and #712 with them |
...
> Both are fixed at #743, one assertion each, driven red separately. Every expectation is still
