...
- **You learned what predicts that nobody will watch a title** → `SIGNALS.md`. It is cited from
  three places in `src/` (`api/routes.py`, `engine/signals.py`, `engine/policy.py`), so read it
  before touching those — and before `engine/backtest.py`, which holds the rewatch curve
  `SIGNALS.md` tabulates (`FALLBACK_REWATCH_PRIOR`). `engine/calibration.py` holds only the
  machinery to fit a per-operator replacement, and that has no caller in `src/`.
