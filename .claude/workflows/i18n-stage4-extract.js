...
//
// args: an array of surfaces, each { name, namespace, files, note? } with files spelled
// repo-relative ("frontend/src/components/JobsPanel.tsx"); `note` carries a standing
// decision the agents must honor (a frozen legacy map, a deferred sibling). Keep a run
// inside the session's agent guideline: each surface costs two agents.
//
...
${s.note ? `
Surface note, binding (text honoring it is not a leftover): ${s.note}` : ''}`
