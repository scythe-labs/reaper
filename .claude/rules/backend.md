...
**94. Every `WHERE col IN :keys` over a scan-sized set is chunked on `db.KEY_CHUNK` (500).** An
...
`test_repo_hygiene.py` collects every membership filter in `src/` and fails on one carrying no
written classification. A new `parent_rating_key`-style filter also needs its covering index.
