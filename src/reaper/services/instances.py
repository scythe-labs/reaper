...
#: rating keys, and Reaper connects to exactly one Plex. A second Tautulli could only
#: double-count the same server's history or carry rating keys from a Plex Reaper never
#: reads, so it is refused at creation rather than silently ignored by the scan. Enforced
#: in two places that both create instances: :func:`create_instance` (the UI) and
#: ``services.seeding.seed_instances`` (the environment); the scan then reads the one
#: Tautulli as a singleton safely.
...
class InstanceError(Refusal):
    ...
    The status is declared here, not at the ``except``. Each subclass below carries the
    status it means, so the class itself is the discriminator and a raise site cannot pass
    the wrong one.

    422 is the base (``Refusal``'s own default): the request was well-formed but the
    service refused its content, which is what a blank required field is. A subclass
    meaning something else overrides its own ``__init__`` to default ``status``
    differently, and a subclass that says nothing inherits the answer that blames the
    payload rather than one that invents a resource state nobody checked.
    """
...
class InstanceConflictError(InstanceError):
    ...
    404, since the request was well-formed and the target exists, it just cannot be
    applied."""
...
def decode_library_map(raw: str | None) -> dict[str, str]:
    ...
    resolver reads ``{}`` as "no library declared" and keeps a duplicated title, since a
    stored config must never be able to crash a scan.
    """
    if not raw:
        return {}
    try:
...
def suggest_library(root_path: str, section_paths: Mapping[str, Sequence[str]]) -> str | None:
    ...
    A prefill only, since the operator confirms it in Settings, so this may guess where
    the resolver never would. ``section_paths`` is ``{library title: [that library's
    folder paths]}``. Each library is scored by the deepest trailing-segment overlap
    between the root folder and any of its folders, the same mount-root-agnostic
    comparison the resolver uses (``/tv-4k`` and Plex's ``/media/tv-4k`` share the
    ``tv-4k`` leaf). The strictly-deepest library wins; a tie or no overlap suggests
    nothing, so the operator picks rather than getting a coin-flip.
    """
...
def _encode_library_map(mapping: Mapping[str, str] | None) -> str | None:
    ...
    ``"{}"``, so "no mappings" is one state, not two: the resolver and
    :func:`decode_library_map` both read NULL and ``{}`` identically as "no map."
    """
    if not mapping:
        return None
    cleaned = {
...
def decode_service_instance_map(raw: str | None) -> dict[str, int]:
    ...
    Keys are ``"sonarr:0"`` / ``"radarr:0"`` style, since Seerr numbers its Sonarr and
    Radarr services in separate lists (both from 0), so the kind is part of the key.
    Values are the Reaper instance id each service adds media to. A missing, malformed, or
    wrong-typed body reads as an empty map, never a crash, and exactly as harmless as an
    absent one: ``build_map`` then falls back to the loose tmdb/tvdb union, since a stored
    config must never be able to crash a scan. A value that is not a positive int is
    dropped, since it can never match a real instance id.
    """
    if not raw:
        return {}
    try:
...
def _encode_service_instance_map(mapping: Mapping[str, int] | None) -> str | None:
    ...
    Blank keys and non-positive-int values are dropped, and an empty result stores NULL
    rather than ``"{}"``, so "no mappings" is one state, not two:
    :func:`decode_service_instance_map` reads NULL and ``{}`` identically as "no map."
    """
    if not mapping:
        return None
    cleaned = {
...
async def create_instance(
    ...
    *,
    ...
    They are accepted at creation because the add form maps the service before saving it:
    the connection test hands back the root folders, so a first *arr can be told apart
    from a second one on the screen that adds it, rather than only on a later edit."""
    name = name.strip()
...
async def update_instance(
    ...
    *,
    ...
    if plex_library_map is not None:  # a dict (even empty) replaces; None keeps the stored map
        ...
        # "there was never a map" are the one state the resolver reads as "no library
        # declared."
...
def _causes(exc: BaseException) -> list[BaseException]:
    ...
    still reachable. Keying the operator message on those types beats sniffing the text
    of a message that upstream is free to reword.
    """
...
def explain_failure(kind: InstanceKind, exc: BaseException) -> Reason:
    ...
    logged by the caller either way, so nothing is lost; it is just not put in front of
    someone who is only trying to get a URL and a key right.

    Every branch adds something a raw client error does not know on its own, such as
    which status means "check your key" versus "check the URL," the certificate advice,
    and the generic-but-deliberate wording for a codeless ``IntegrationError``, so each
    keeps its own ``error.instance.*`` code rather than nesting the client's. A code is
    reused across call sites only where the English would be the same; this English never
    is.
    ...
    if isinstance(exc, IntegrationError):
        # The server answered and reported a problem of its own, with no HTTP status to go
        # on. This is the commonest Tautulli misconfiguration: it answers a bad API key
        # with a normal HTTP 200 whose body says the request failed. It also covers an
        # answer in a shape Reaper could not use, and a URL that redirects in a loop. The
        # raw text stays in the log; the key and the URL are what an operator can act on.
...
def _client(
    ...
    *,
    ...
    # (scan_runner.build_sources), and leaving it off here would let a test pass against a
    # path the scan does not use the moment the column ever diverged. ``None`` keeps each
...
async def test_connection(
    ...
    *,
    ...
    the same path the scan will. Nothing writes that column today, since it has only ever
    held its default, so a server that does not serve ``/api/v3`` cannot be reached by
    editing it. A reverse-proxy subpath is fixable, though: the base URL is merged into
    every request, so storing ``https://host/sonarr`` works.
    """
    try:
    ...
    try:
        async with client:
            ...
            # 401/403 lands in explain_failure's key-refused branch.
...
async def probe_root_folders(
    ...
    *,
    ...
    ``instance_root_folders`` below cannot reach it. Without this the folder map would
    only appear on a service that was already saved, which is never where a first-run
    ...
    ``section_paths`` is the Plex side (``{library title: folder paths}``), passed in by
    the caller so this function needs no Plex client of its own. Raises
    ``InstanceError`` for a wrong kind, and lets the arr client's error surface for a
    connection failure.
    """
...
async def instance_root_folders(
    ...
    *,
    ...
    """This stored instance's root folders, each with a suggested Plex library (a prefill
    only).

    The saved-row half of ``probe_root_folders`` above, which holds the whole read: one
    implementation, so the folder list and its suggestions cannot come to differ between
    the add form and the edit form. Raises ``InstanceError`` for a wrong kind or a
    missing instance, and lets the arr client's error surface for a connection failure
    (the caller maps it).

    ``api_path_prefix`` rides along for the reason ``test_connection`` gives: this probes
    the same path the scan will use.
    """
    row = await _get(session, instance_id)
...
def _host_port(url: str) -> tuple[str, int]:
    """A URL's ``(lower-cased host, port)`` with the scheme's default port filled in."""
...
def _suggest_instance(service: SeerrService, arr_rows: Sequence[Instance]) -> int | None:
    ...
    A prefill only: matched on kind, host, and port, and suggested only when exactly one
    Reaper instance matches, so an ambiguous or missing match leaves the operator to pick
    rather than get a wrong default. Seerr and Reaper often reach the same server at
    different addresses (a container hostname vs. a LAN ip), so a miss here is expected,
    not an error.
    """
...
async def probe_seerr_services(
    ...
    *,
    ...
    The add form's half, exactly as ``probe_root_folders`` is for an *arr: the connection
    test calls this on a pass, so the requester map arrives filled in rather than only
    appearing after a save. ``arr_rows`` is the suggestion pool, passed in so this needs
...
async def seerr_services(
    ...
    """This stored Seerr portal's Sonarr/Radarr services, each with a suggested Reaper
    instance.
    ...
    The live ``/settings`` read exercises the stored (admin) key. Raises
    ``InstanceError`` for a wrong kind or a missing instance, and lets the Seerr client's
    error surface for a connection or non-admin-key failure (the caller maps it).
    """
    row = await _get(session, instance_id)
