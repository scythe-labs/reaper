...
A backup is one gzip-compressed tar with up to five members:
    ...
    secret.key      the master key that decrypts every stored credential. Present
                    ...
                    supplies ``REAPER_SECRET_KEY`` from the environment, since then
                    it is never written to disk and there is nothing to bundle
...
and :mod:`reaper.secrets`), so a database-only backup would silently lose every
...
fenced off from the API-key lane and the operator copy says to guard it like a
password.
...
one defragmented file with no side ``-wal`` or ``-shm``, and drops free pages so the
archive is smaller than the live database on disk.

Two things build one of these archives: the operator pressing Download, and
:func:`snapshot_before_migration`, which runs at boot when a pending schema change
asks to be snapshotted first. The second keeps its archives in
``data/pre-migration/`` and prunes to the newest few; both write the same format,
so either restores through the same Settings -> Backup card.
"""
...
#: than no backup, because the operator would believe they are covered until the day
#: they are not. 64 GiB is far above any real install: ``reaper.db`` holds decisions and
#: credentials, never media, and the rebuildable cache is not in here at all. The
#: restore's extraction is streamed and capped as it runs, so raising this ceiling costs
#: nothing. It is a decompression-bomb bound, not a sizing estimate.
...
#: three name transient work that a healthy run removes itself; anything matching them at
#: boot is crash debris that :func:`sweep_stale_temp` clears.
...
#: mounts, already backs up, and already knows to look in, and because moving a file out of
#: the build's temp dir is then a rename on the same filesystem rather than a copy of the
#: whole database. These are ordinary ``.reaper`` archives, so recovery uses the same restore
#: the app already has (Settings -> Backup, which accepts exactly this file) rather than a
#: second path written for this one case.
#:
#: Three, covering the upgrade that just went wrong, the release before it, and one to
#: spare. Each one is roughly the size of ``reaper.db`` compressed, and they are only ever
#: written by a schema change that asks for one, so the folder does not grow on its own
#: between releases.
...
#: the code that fails, and printed by :mod:`reaper.preflight`, which turns it into a
#: refusal. It says the update did not run, which preflight makes true by returning
#: non-zero before ``alembic upgrade head``, and it claims nothing about the state of the
#: data, because the staged-restore swap runs above this and may already have replaced it.
#:
#: It asks the operator to check the free space rather than telling them to make some. A
#: full disk is the usual cause, but not the only one: :class:`BackupTooLargeError` fires
#: on a database past :data:`MAX_DB_BYTES` even with all the space in the world, and "free
#: up disk space" would then be an instruction that cannot work. Preflight appends the
#: underlying error, which is where the exact cause is.
SNAPSHOT_FAILED = (
...
class BackupTooLargeError(RuntimeError):
    ...
    Refusing beats writing a file the restore side will not accept: the operator finds
    out now, while they still have every option, rather than during a recovery.
    """
...
class BackupArchive:
    ...
    ``tmp_dir`` is the caller's to remove once the file has been streamed out (see
    :func:`cleanup`); everything else describes the archive.
    """
...
def db_size_on_disk(base: Path) -> int:
    """The size of one SQLite database on disk, including its live write-ahead log.
...
#: the boot gate asks the same question of the live database, and a second copy of the
#: query would be a second answer to drift from. ``None`` means no ``alembic_version`` row:
...
def _build_sync(settings: Settings, created_at: str) -> BackupArchive:
    """The blocking build: snapshot the database, then tar and gzip it with the key files.
    ...
    # Anything past mkdtemp that raises, such as VACUUM INTO failing on a full disk, a
    # database still locked past the 5s busy timeout, or a gzip write error, must not
    # strand the temp dir and its partial multi-GB snapshot, which would make a
    # disk-full situation worse.
    try:
...
def _build_into(settings: Settings, created_at: str, tmp_dir: Path) -> BackupArchive:
    ...
    # failing at once. This timeout belongs to this connection alone, not the app engine's.
    ...
    # bundle it, and report key_source "env" so the target knows it still needs the
    # environment variable set. The salt is install state, minted even for an env key, so
    # it travels whenever it exists.
    ...
    # a backup without it would restore an install that has forgotten every one of them; the
    # database carries none of it. Carried whenever the file exists, rather than gated on
    # the install shape, so a backup taken anywhere holds everything that shape stored. A
    # container that restores it simply never reads the file
    # (`launcher.reads_launcher_conf`).
...
def _prune_pre_migration(directory: Path) -> None:
    ...
    Sorted by name, which is sorted by time: the stamp is ``YYYYMMDDTHHMMSS``, so
    lexicographic order is chronological order.

    Nothing in here raises, and the listing sits inside the same guard as the unlink for
    that reason. This runs after the snapshot is already on disk, so an exception escaping
    it would reach preflight's refusal and stop the boot under :data:`SNAPSHOT_FAILED`, a
    sentence saying the backup could not be saved when it actually was. Losing a boot over
...
def snapshot_before_migration(settings: Settings, revision: str | None) -> Path:
    ...
    Called from :mod:`reaper.preflight`, which every way of starting Reaper runs
    immediately before ``alembic upgrade head``: the container entrypoint,
    ``scripts/dev-local.sh``, and :func:`reaper.launcher.main` alike, so one call site
    covers all three and none of them can drift out of it. Which migrations ask for this
    is :func:`reaper.db.schema_gate.needs_snapshot`.

    Raises rather than returning on failure, and preflight turns that into a refusal: a
    migration that can destroy data must not run with nothing to go back to, so a full
    disk stops the boot instead of taking the update unprotected.

    ``revision`` is the revision the database sits at, and it goes in the filename because
    that is the fact an operator needs when picking which snapshot to restore. The archive
    is a normal backup, key material and all, so it restores through Settings -> Backup
    like any other.
    """
    created_at = utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
    archive = _build_sync(settings, created_at)
    try:
        # 0700 at creation, never widened afterward: each archive carries `secret.key`, so
        # the folder holding them must stay as protected as the key itself. `mkdir` only
        # masks with the umask, which can remove bits but never add them, so this is a
        # ceiling, not a floor.
        ...
        # keeps it owner-only if the operator later moves it somewhere that is not.
...
def sweep_stale_temp(settings: Settings) -> int:
    """Remove crash-leftover backup or restore temp entries under ``data/``. Returns the count.
    ...
    Called from preflight, which runs before the app starts, so nothing is in flight to
    race. Only the dotted temp prefixes match, so the ``pending-restore`` staging, the
