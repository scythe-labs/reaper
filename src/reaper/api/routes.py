...
def _refused(kind: SimStale, media_type: str) -> SimulationOut:
    ...
    The sentence lives here alone and the panel renders it (``PolicySimulator.tsx`` supplies
    only the heading). It used to live in both, one copy per side, "kept in step" by a comment
    and by nothing else -- and the operator never saw this one at all, so the copy that was
    read and the copy that was reviewed were different strings. Rule 144: the way to keep two
    statements of one fact agreeing is to have one of them.
    ...
    reasons = {
        SimStale.GATHERS_DIFFERENTLY: (
            f"Your {lane} policy asks the scan to read something different: a keep tag, or how "
            "far back watching counts. Run a scan to apply it, then this becomes exact again."
        ),
...
def _season_guard_replay(
    ...
    *,
    ...
    replaced = [replayed if r.gate is GateId.SEASON_PROGRESSION else r for r in frozen]
    if len(replaced) == len(frozen) and not any(
        r.gate is GateId.SEASON_PROGRESSION for r in frozen
    ):
        raise _SeasonEvidenceMissingError(SimStale.SEASONS_NOT_RECORDED)
    return replaced
...
async def simulate(request: Request, payload: PolicyIn) -> SimulationOut:
    ...
    **The TV lane has a second precondition the hash cannot express.** A season's guard is
    ...
    (``db.models.SeasonPruneEvidence``), and a snapshot predating that table has none while
    its evidence hash matches perfectly. Turning the mid-binge hold ON over a scan that ran
    with it off is the same shape: the policy gathers identically, and Sonarr's episode
    lists were never read. Both are asked of the stored evidence in ``_season_bundles`` and
    ``_season_guard_replay``, and each refuses with its own ``SimStale`` so the panel names
    the control at fault instead of blanking nine of them (#491, #495).
