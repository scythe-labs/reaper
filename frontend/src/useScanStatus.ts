 ...
 *
 *  This observer polls only while a scan is running. It does NOT follow that the query goes
 *  quiet the rest of the time: React Query runs one timer per observer, and `Dashboard` holds a
 *  second observer on this key with a 15s idle poll, so anywhere below the app shell the key
 *  keeps being read whatever this returns (#712). The setup wizard is the exception, because
 *  `Authed` renders the wizard OR the Dashboard, so there the polling really does stop.
 *
 *  Two readers deliberately do not call this. `Dashboard` declares the idle poll itself, which is
 *  a different job: it is what surfaces a scan the scheduler or another device started. And
 *  `ReapBreakdown` reads the cache with no interval at all, so the ledger costs no request of its
 *  own. */
