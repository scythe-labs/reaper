...
//
// There is no type information available to a syntax-only scan, so "error-shaped" is a
// naming convention rather than a type check (rule 147: a matcher is bounded by the population
// it can parse, so it must be proven against every SPELLING the tree actually writes this in,
// not just the ones a first draft happens to catch). The sweep that converted every site this
// gate now holds found exactly these shapes: a bare identifier named `err`, `error`, or `e`, or
// one ending in "Error" (`keyError`, `invalidError`, `simError`, `webUrlError`, ...), and a
// property access whose own name is `error` (`mutation.error`, `save.error`, `check.error`,
// `testSaved.error`, ...) -- react-query's own field name for a failed query or mutation.
