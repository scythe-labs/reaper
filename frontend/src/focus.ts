...
//
// Where focus goes when a surface opens and when it closes, written once instead of answered
// per call site.
//
// The failure is always the same shape, and always invisible to a sighted mouse user:
// `document.activeElement` falls back to `<body>`, and the next Tab restarts at the top of
// the document.
 ...
 *  (WebKit's `determineIsAccessibilityElement`, confirmed by a committed layout test), so
 *  focusing the spare-length menu's container put the VoiceOver cursor nowhere, and a swipe
 *  walked the panel behind the menu instead of over its own rows. Neither `tabIndex={-1}` nor
 *  the portal fixes that; landing on a real control does, the APG Menu Button pattern. Default
 *  is the container, which is right for a dialog and is what the other two callers keep. The
 *  close-restore is unaffected either way: `entry` is inside the panel, so the `contains`
...
export function useDialogFocus(
  ...
  //
  // This one is declared first so it reads `document.activeElement` before the second effect
  // can move it. Reading it after that move would capture the panel itself as the invoker,
  // since focus would already be inside it by then, so closing would try to hand focus back
  // to the very node that is going away.
  //
  // Its deps are `[]` (bar the stable ref), because this cleanup means the surface closed.
  // Keying it on `active` as well would run the close-restore whenever that flag merely
  // flipped, such as the viewport crossing the panel's media query when a phone is rotated,
  // yanking focus out of a panel that was still on screen and back to the card behind it.
  useEffect(() => {
  ...
  // Moves focus in, and re-enters if the surface becomes active later: a panel the operator
  // opened on a wide screen and then narrowed into a full-screen sheet is covering the app
  // from that moment, so it takes focus from that moment. Going the other way, it simply
  // stops trapping and leaves focus where the operator has it, since there is nothing to hand
  // back to while it is still open.
  useEffect(() => {
 ...
 *  the same list takes this one instead of writing a second. `trapTab` in ModalShell is its
 *  only reader today. */
 ...
 *  next Tab restarts at the top of the document, on the policy page a form roughly 1,900
 *  lines long. An operator removing three tags in a row would be thrown to the top three
 *  times.
 *
 ...
 *  no-op as focusing a `disabled` control (`OverrideControls`' `returnToCaret` hits that one).
 ...
 *  Focusing one is a silent no-op: the browser drops it straight back to `<body>`, the same
 *  trap `OverrideControls`' `returnToCaret` hits. Read from both ends by `useSuccessorFocus`. */
 ...
 *  which is the right shape for a draft list edited in the browser and the wrong one for
 *  anything waiting on a write. Most of the sites this was written for also carry
 *  `disabled={…isPending}`, so the browser has already dropped focus to `<body>` at the press,
 *  before the unmount even happens; there is nothing left to move by the time the successor
 *  exists.
 *
 *  So the request is armed in the handler and every commit afterwards gets a turn at
 *  satisfying it. Three settle timings fall out of one hook: a successor that mounts with
 *  `onSuccess`, one that waits for the invalidated query to refetch, and one that passes
 *  through an intermediate state where the only candidate is still `disabled`. That is why
 *  being mounted is not enough, and a disabled target is waited out rather than focused.
 *  Focusing a disabled control is the same silent no-op `OverrideControls`' `returnToCaret`
 *  hits.
 *
 *  Used as:
 *
 ...
 *  a failed refetch leaves it armed, and only the next commit that finds focus lost can spend
 *  it.
 *
 *  "Lost" is broader than `<body>`, and deliberately so. A browser blurs an element that
 *  becomes `disabled`, so on the sites whose press disables the control it is on, `<body>` is
 *  what a real browser reports. But jsdom does not blur it, and leaves the cursor parked on a
 *  control that can no longer be acted on. Reading only `<body>` would make this hook behave
 *  one way in the test suite and another in the app, which is the worst of both. A control
 *  that is gone, or still there and no longer actable, is nowhere to stand either way. */
 ...
 *  control the operator just activated: focus falls to `<body>` and the next Tab restarts
 *  above the whole form, roughly 1,900 lines long on the policy page. Both presses already
 *  announce the result ("Movie policy saved.", "Changes discarded."), so what is missing is
 *  only the place to stand.
 *
 *  That place is the panel's own heading, chosen for what it is rather than for being nearby:
 *  after a completed action a screen-reader operator wants the name of what they are looking
 *  at, a fixed, findable point rather than wherever the bar happened to be. Pass the heading a
...
export function useSavebarFocus() {
  ...
  useEffect(() => {
    ...
    if (document.body.contains(headingRef.current) && !document.querySelector(".savebar")) {
      wanted.current = false;
      // `preventScroll` is the point of this call, not a nicety.
      //
      // Focusing an element scrolls it into view by default, and the target here is
      // deliberately a fixed point near the top of a form that runs to nineteen hundred
      // lines, not somewhere near the operator. Without `preventScroll`, Save and Discard
      // would both throw the page back to the heading, however far down the operator had
      // scrolled to make the edit they just saved: the two presses that mean "I am done with
      // this bit" would be the two that lose their place.
      //
      ...
      // stays untouched.
      //
      // The trade: a sighted keyboard operator who scrolled down the form now holds focus off
      // screen, so no focus ring is visible anywhere until their next Tab, which lands at the
      // top. Landing them somewhere visible instead needs a target already in the viewport
      // when the bar goes, and the bar is `position: sticky` at the very end of the form, so a
      // sentinel in its flow position is not one, it is the bottom of the page. That needs a
      // mockup, not a one-line swap.
