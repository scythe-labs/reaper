...
// share.
//
// A hook that opened its own query per caller (one per Spare/Reap control, on each card and
// each season row of every expanded show, plus one per card status line) does not scale: four
// hundred drawn cards with their seasons open come to roughly a thousand query observers on
// two keys. The request is deduped, but every write to either cache then re-renders all
// thousand. The queue subscribes once, provides the values here, and a write now re-renders a
// row only when the value that row reads has actually changed.
//
...
/** The two stored settings every row in the queue reads, subscribed to once. See the file
 *  header for why: a per-caller query does not scale past a few hundred cards.
 *
