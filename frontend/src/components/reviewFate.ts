...
//
// These are the shared primitives behind that question, and the whole app (the queue, the why
// panel, the show panel) has to agree on them, so they live here in their own module. Nothing
// in this file renders or fetches: it reads the fields a candidate already carries and answers
// one question each.
//
 ...
 *  A hand SPARE, or a reap the engine honors, shows SOLID, meaning "you chose this", so the
 *  cell states the item's real fate, not just Reaper's first read. A reap the engine can't
 *  honor yet ("refused") reads DASHED RED, never solid: the ask is noted, the file is still
 *  held. Amber is used only for "left for you to decide" (the abstain `status-look` chip),
 *  never for this. Anything untouched keeps its scan verdict. Shared by the score badge and
 *  the season strip so the two can never disagree with each other or with the row's chip.
 *
 *  `"spare-expired"` is the fourth: a hand spare whose clock has passed. It is DASHED GREEN,
 *  and it is neither of the two states it sits between. It is not solid green, because the
 *  operator's decision has run out, and solid means "you chose this and it holds". It is not
 *  the scan verdict either, because the item is genuinely still kept: only a scan realizes a
 *  spare's expiry (`whitelist.purge_expired_spares`), so until then the planner, the ledger
 *  and the executor all still read that spare. Painting it by verdict would tell the operator
 *  the item is back on the block when nothing will reap it. The dashed treatment is the one
 *  this app already uses for a decision whose effect is pending a scan, worn today by the
 *  held reap. */
...
export function handFate(item: {
   ...
   *  item's own key beats its show's. Precedence answers which row Reaper is reading. It
   ...
   *  kept forever, and reading the own key alone would paint it dashed "expired" over a file
   ...
   *  `"spare"` and paints the solid "you chose this" green, which is the one color it must
   *  not wear. That is the pre-expiry behavior and the keep direction, so a missing value
   *  fails safe rather than loud, which is exactly why a new fate-bearing surface has to be
   *  checked against this line rather than trusted to error on its own.
   *
   ...
   *  that treats the two spare states differently is a bug the types cannot catch, so every
   *  consumer of this function's result must enumerate both spare states, or say in its own
   *  doc why one answer covers them. */
 ...
 *
 *  It speaks for ONE item. A show is in every lane one of its seasons is in, so a caller
 *  opening a whole group names the lane it means rather than asking this. */
...
export function isCondemned(item: {
  ...
  // The item's effective fate (will it be removed), not its raw, pure-policy verdict: a hand
  // spare and a held hand reap keep it, and an honored hand reap condemns it. Asked as "is its
  // lane the condemned one", so the card's styling and the tab it can be found on are one
  // fact, not two that can drift. Reaches `handFate` through `laneOf`, so it still cannot
  // disagree with the colors. This answers a different question from `hideReap`; see
  // `reapIsNoop` for that one.
  return laneOf(item) === "condemn";
}

/** Whether a hand Reap on this row would change nothing, so its Reap control is dropped.
 *  Reaping is a no-op only when policy already condemns the item (its pure verdict is
 *  "condemn") and the operator has not spared it: a spared row, including a season kept by a
 *  whole-show spare, can still be flipped to Reap, and a row condemned only by the operator's
 *  own reap keeps that control so the reap can be undone (its pure verdict is not "condemn").
 *  This is the spare-aware form of the "own verdict === condemn" test. Never reimplement it
 *  inline. */
 ...
 *  wording: "all" every inherited reap is honored, "none" every one is held for now, "some" a
 *  mix. Reads the same override/override_effective fields as groupReapEffective, over the
 *  seasons the panel already has. Only meaningful when the show's own override is reap. */
 ...
 *
 *  A whole-show decision is not atomic. A season carrying its own spare keeps it, and one the
 *  engine refuses to reap (streaming right now, an unreadable protection) comes back
 *  `override_effective: false` and is dropped from both the server's rollup and the planner's
 *  group expansion. Counting the show as a whole would put "3 of 3 would be removed" above a
 *  chip reading "kept for now" and three dashed-red refused squares, a mismatch that would
 *  persist even after the refetch settles.
 *
 ...
 *  whitelist, where an expired row is still a spare until a scan purges it. So this asks "is
 *  it spared at all", not "is the spare live": reading only `"spare"` would count an expired
 *  one as removable and print a number the planner would not act on, beside the dashed-green
 *  square that says the season is kept.
 *
 *  Routed through `handFate`, so the number and the colors beside it cannot disagree, and it
 *  reads correctly in both states the card lives in. Before the refetch, the marks still
 *  carry their pre-decision override (`patchShowOverride` writes only `show_override`, by
 *  design), so nothing reads as spared or refused and the count moves the instant the
 *  operator clicks. After it, the inherited reap and its refusals are on the marks and the
 *  count settles to what the server will really plan. */
export function showReapReaches(season: {
  verdict: Verdict;
  override: Override | null;
  override_effective: boolean | null;
  /** Passed through to `handFate`, which needs it to tell an expired spare from a live one.
   *  Both keep the season, so this changes no answer here. It is declared so a caller cannot
   *  quietly hand over a mark whose expiry the type dropped on the floor. */
  spare_covers_until?: string | null;
}): boolean {
...
/** Whether a whole-show Reap would change nothing: the show analogue of the already-condemned
 *  test for a single row. It decides `hideReap` for a show on both the card and the panel, so
 *  the test lives here once rather than being reimplemented at each surface.
 *
 ...
 *  Passing only the seasons on the current tab would break on the Condemned lane, where that
 *  set is all-condemned by construction, wrongly dropping Reap and hiding the one control
 *  that reaps the show's kept seasons. So the parameter takes the strip-mark shape too (the
 *  rollup's `seasons`), not just a full `Candidate`. */
