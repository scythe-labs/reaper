...
//
//   1. The reasons that pushed toward removing, each carrying its share of the score, not
//      its raw weight. The shares add up to the number beside them, so the receipt can be
//      checked by adding it up rather than by dividing by a total never shown.
...
/** A pill that opens the item in one of the tools that manage it. Hidden without a URL: a
 *  missing link is hidden, never rendered broken. Shared with the show panel. */
...
/** One ratings chip. With a site link it opens the number's source in a new tab, without
 *  one it stays a plain chip: never a dead link. */
function RatingChip({
  href,
  title,
  children,
 ...
 *  slide-out is the one place the plot lives, but it still should not push the reasoning
 *  below the fold, hence the clamp. Shared with the show panel. */
export function Synopsis({ text }: { text: string }) {
  ...
  // too, and the measurement below, which is skipped while open, would go on answering for
  // the previous text. Before paint, so the new title never flashes expanded.
  useLayoutEffect(() => setExpanded(false), [text]);

  // Whether two lines are enough is a question about how wide the panel is, never about
  // how long the string is. A fixed character-count threshold answers this wrong: the width
  // it is right at is not the one a phone has, so measuring by string length either clips
  // text nothing hides or shows a "more" link nothing needs. Ask the element instead, which
  // is also right at a text size or browser zoom nobody predicted.
  //
  // Same shape as `usePopoverShift`: layout rather than paint, so the control is there in
  // the first frame instead of appearing after it, and re-measured on every render plus a
  // window listener, because React re-renders nothing on a resize. Setting the same value
  // bails out of the re-render, so this settles rather than looping.
  useLayoutEffect(() => {
  ...
  return (
    <p className="why-summary">
      ...
      {(clipped || expanded) && (
        <button
          className="link-btn"
          // A disclosure that never said it was one. The clamp is CSS-only (`.clamp-2`), so
          // the whole synopsis is in the accessibility tree either way. Without
          // `aria-expanded`, pressing this would appear to do nothing at all, twice, since
          // the label also flips back. Every other disclosure in this file is a native
          // `<details>` and states itself for free.
 ...
 *  nothing at all. Three plain wordings, no jargon, and they are not interchangeable:
 *  nothing found in Plex, several Plex rows answering to it, or each kind of evidence
 *  naming a different single row, which is the two apps describing one file differently
 *  and says nothing about how many copies Plex holds. Reuses the shared amber
 *  `.notice-warn` tone, like every other "we could not look" state.
 *
 *  Branches on the typed `status`, never on the wording, and that is also why the
 *  candidate links hang here rather than off the matching "Left for you to decide" box,
 *  whose rows are grouped by a cause string and could only be picked out by substring. */
function KeptNotice({
  match,
  links,
  mediaType,
  ...
  return (
    // `standing`: `match.status` is frozen into the scan's explanation, so it is true every
    // time the panel is opened on this title. Its candidate links are focusables a
    // component deep (`MatchCandidates` -> `JumpPill`), so they are reached by navigating
    // rather than by being interrupted, and the same rows render bare at `ShowPanel.tsx`,
 ...
 *
 *  Quiet and neutral, on the shared `.chip` base with the other facts on this line (no new
 *  CSS needed): how many times a file is listed is a fact about the library, never an
 *  argument for its fate. Silent below two, which covers both the ordinary single-listing
 *  bind (the server sends null) and a record stored before the field shipped. "Listed 1
 *  time" on every item would be noise, and this line is scanned.
 *
 *  Worth a chip at all because the count is load-bearing on the deletion path: the executor
 *  re-reads `merged_rating_keys`, so a watch on any of these listings counts for the file
 *  and all of them go together. The chip carries the number and the `title` carries that
 *  consequence, which is the same split `LibraryChip` already uses.
 *
 *  Not on `ShowPanel`: its group has no `match` record, so there is no count to read there.
 *  Carrying it over is a backend change (a merged count on the group payload) and is
 ...
 *
 *  Every other jump link on this panel is built from the item's own rating key, which is
 *  null on exactly the rows this notice appears over. Without this, the panel would name a
 *  problem in the owner's Plex and offer no way to open it. Reaper knows nothing about
 *  these rows but their keys, so they are numbered rather than named. A single candidate
 *  drops the number, since "Plex 1" alone reads as a label for something it is not.
 *
 *  Exported because `ShowPanel` renders the same rows under the same kind of sentence: a
 *  conflicted show lands there, its title link is `group.links.plex`, and that is null for
 *  exactly these rows, the identical dead end this component exists to close. */
export function MatchCandidates({ links }: { links: Links }) {
  const { t } = useTranslation();
  const candidates = links.match_candidates ?? [];
  // Two ways to have nothing to offer, and only the first was checked here. `JumpPill`
  // renders null for a null href, so a row whose candidates all lack both a Plex and a
  // Tautulli link (no Plex server row or no `plex_web_url`, and no enabled Tautulli, at
  // render time) would leave the lead sentence standing over an empty row, ending on a
  // colon. This closes that dead end too.
  ...
  return (
    <span className="match-candidates">
      <span className="match-candidates-lead">
        {/* The count decides the noun. The component already asserts a single-candidate
            case exists (`numbered` drops the number for it), so the lead text must not
            read "1 possible matches" beside it. */}
        {t("why.panel.matchCandidates.lead", { n: candidates.length })}
      </span>
      {candidates.map((candidate, i) => (
        // The rating key: unique among siblings and a stable server id, where the index
        // would renumber every pill if the list ever reordered.
 ...
 *
 *  Read the gate's typed `unestablishable` flag, never the sentence text: the same prefix
 *  ("could not check...") can open three different conflict shapes, so matching on wording
 *  cannot tell the caller which one it has. `GateOutcomeOut` now serves that flag, and
 *  `verdictLook` branches on it from the outcome.
 *
 *  A `season_progression` row reaches `protections_unknown` three ways, and
 *  `unestablishable` is what tells them apart. The conflict arm is the one this function
 *  wants. The second is a season the guard protects but could not answer
 *  (`ProtectedSeason.unestablishable`). It never reaches this function anyway, because a
 *  fired protection makes the verdict `protect` and the Sanctuary branch returns on it
 *  before this runs. The third is the mid-binge check on a show Plex never resolved, which
 *  asked nobody and abstains beside the four Plex-dependent gates that say why, landing in
 *  this same abstain branch. Only the explicit flag keeps it out: reading it as a conflict
 *  would say "Reaper couldn't check who watched these seasons, so it left the call to you",
 *  offering a decision on a comparison that was never attempted.
 *
 *  `undefined` and `null` both mean a row scanned before the flag existed, and both read as
 *  a conflict, because before the flag that is the only thing an abstaining item's season
 *  guard put here.
 *
 *  This arm is reachable often, not an edge case: it fires wherever the watch history is
 *  shallower than the library is old, which is the default rendering for TV on such a
 *  server. */
 ...
 *    denies in the producer's own "Reaper cannot tell whether Season N ...". Three whole
 *    sections separate them (the score breakdown, the keep leanings, the cleared
 *    protections), which is a large part of why the contradiction went unseen: nobody had
 *    both on screen at once. Shares its wording with the card's chip, so the queue and the
 *    panel agree.
 *  - absent: nothing legible in the row distinguishes the two shapes, so it names neither.
 *    Two rows land here: one frozen before the flag shipped, and one carrying a value that
 *    is not a bool at all, which the server reads to `null` rather than coercing or
 *    refusing (`engine.gates.thaw_defers_to_owner`). Vague and true beats specific and a
 *    coin flip. The next scan resolves either into one of the two above. */
 ...
 *  which is something playing right now and the retired `unmanaged`. Neither is a judgment
 *  about whether the file is wanted, which is the whole test: deleting mid-stream breaks a
 *  session someone is watching, and a file no *arr manages has no path to delete through.
 *
 *  Every other protection is cautious rather than structural, and a hand reap condemns
 *  straight past it (`docs/DECISIONS.md` under "What a hand reap may overrule"). This is
 *  the one test that separates "kept no matter what" from "kept unless you say otherwise",
 *  and both sentences below turn on it: `heldReapNote` says why a reap is still held, and
 *  the Sanctuary note says whether one would be. Deriving them apart is how the panel can
 *  call a protection absolute beside a working Reap button. */
 ...
 *  (`engine/verdict.py`).
 *
 *  `structuralStop` is checked first, then the match status: an item can have more than one
 *  of these reasons at once, but only the first one found is shown, so the check order
 *  decides which reason the operator reads. The card chip beside it checks in the same
 *  order (`api.review._chip`), so the queue and the panel never disagree about why a reap
 *  is held. */
 ...
 *
 *  It reads the spare that covers the item, not the one in force by precedence, because
 *  every sentence here asserts what will happen to the file. A season spared ten days
 *  inside a show spared forever was told "10 days left, then Reaper judges it again", and
 *  Reaper does no such thing: the season's spare lapses and the show's goes on keeping it.
 *
 *  The expired arm is the one that has to be careful. The item really is still kept: only a
 *  scan realizes a spare's clock, so this must not read as "it will be removed now". It
 *  says what happened and what is still true, in the same words the button's tooltip uses,
 *  from the one derivation (`spareRemaining().note`). */
function spareNote(item: CandidateDetail): string {
  const cover = spareRemaining(item.spare_covers_until);
  // A show-level spare outlasts this item's own exactly when the two fields disagree: they
  // are the same date whenever the item's own spare is the one covering it, both being one
  // `expiries` lookup on the same key (see `whitelist.covering_spare_expiry`). Comparing
  // them beats re-deriving "is the show's longer" on the client, which would be a second
  // derivation, and it needs no date math at all.
 ...
 *  frozen scan verdict: a hand reap the engine honors reads as a removal, a reap it can't
 *  honor yet reads as held (dashed red, never solid), and a hand spare says the owner kept
 *  it, so a reaped or held item is never mislabeled "Sanctuary". Only with no hand decision
 *  does the scan verdict speak, and then "Sanctuary" is claimed only when a protection
 *  actually fired (a protect with nothing fired is a stale held-reap row from before this
 ...
 *
 *  A press is the state that survives. Hover only previews. On a phone there is no hover at
 *  all, so the press is the whole interaction and the row behaves as a plain expander: tap
 *  to open, tap to close. It expands in place rather than floating, which also keeps it
 *  clear of the edge-clipping problem an anchored popover would have: inside a full-screen
 *  sheet, a popover has nowhere to be pushed back to.
 *
 *  The hover pair is bound only where a real pointer exists, because a touch tap
 *  synthesises `mouseenter` before it sends `click`. Bound unconditionally, the two
 *  handlers would fight, on exactly the device that can use neither. `useMediaQuery`
 *  reporting `false` under jsdom is the safe way round here: press-only is the behavior
 *  that works everywhere.
 *
 *  There is deliberately no focus handler. A press focuses the button on every platform, so
 *  opening on focus too would leave `previewing` true underneath the press: the second tap
 *  would unpress a row that stayed open, and the control would read as dead. Enter and
 *  Space already arrive here as a click, so the keyboard is served by the press like
 *  everything else, and nothing can strand a row open behind a Tab.
 *
      ...
      <span className="sig-head">
        ...
        <span className="sig-share">
          {/* The one middot in the app that may not simply be hidden. Everywhere else the
              character separates two facts that survive without it. Here it is the fact:
              the whole of what this column says for a reason Reaper could not read, so
              hiding it alone would leave the column silent while every sibling row reads
              out "+5", "<1" or "0". It gets the word instead, in the same vocabulary as the
              group's own heading. On the panel an operator reads before approving a
              deletion, a row that could not be checked is the last one that should go by
              unheard. */}
          {state === "unreadable" ? (
            <>
...
/** How much of the evidence was checkable, out of 10,000, when every weighted reason was
 *  readable. Stored in these units because the policy body is integers-only (see
 *  `format.coverage`), so full is 10,000 and not 1. */
 ...
 *  Never applied to "Couldn't check": more reasons Reaper could not read is more cause to
 *  look. */
...
function Signals({ item }: { item: CandidateDetail }) {
  ...
  // Below the floor, name the line coverage fell under, the way the score names its
  // threshold. An abstain forced by too little readable evidence scores at or above the
  // threshold and is held anyway (`decide_verdict` checks coverage before the score), so
  // the score alone never explains that hold, and the coverage number alone does not say
  // it was too low. This only renders when the two round to different percentages: "40%,
  // under the 50% it needs" reads fine, but "50%, under the 50% it needs" would read as a
  // bug. Silent when the floor predates the field (null, so the panel omits it like a null
  // threshold) or coverage cleared it. `coverage_bp < floor` implies `< FULL_COVERAGE_BP`,
  // so this clause only ever rides inside the sentence below.
  ...
  return (
    <section className="block">
      ...
      {/* The coverage clause is silent at full coverage. The only thing that can lower this
          number is a reason Reaper could not read (`SignalState.UNREADABLE` is the one
          state that costs coverage), and those get their own "Couldn't check" group below
          with their own note. Showing it at 100% would announce the absence of a group
          that is already absent: a number with nothing behind it. Below full it is the one
          place the panel says how much of the scoring weight went unread, which the
          group's row count does not answer: one unread reason out of five can be most of
          the score. */}
      <p className="blurb">
 ...
 *  verbatim either way (docs/history/I18N_PLAN.md §5). Empty where the row carries no
 *  detail at all. */
          ...
          return (
            <li key={`${outcome.gate}:${text}`} className={custom ? "gate-custom" : ""}>
              {/* Decoration: every row in this list is a pass, so the tick adds nothing a
                  ...
                  hold both outcomes (the reap report's per-item checks), the glyph is
                  hidden and a word carries it instead. */}
...
/** What goes stale the moment one title's watch record is forgotten, each with the surface
 *  it feeds. Read off the queries themselves rather than guessed: this panel is fed by
 *  `["candidate", id]` (App.tsx) and the queue by its `["candidates"]` pages (ReviewQueue),
 *  so both re-read. `["watch-evidence"]` is the Settings count of stored records, which
 *  really does drop by one here, the same key its global Forget invalidates (PlexPanel).
 *
 *  This does not re-judge the row: the verdict and its reasons are frozen snapshot data, so
 *  the refetch returns the same explanation and the notice stays up until a scan reads the
 *  title again. Nothing here says otherwise, and the button's help text claims only which
 *  plays get used. */
const WATCH_RECORD_STALE = [["candidate"], ["candidates"], ["watch-evidence"]];

/** The Stage 2 rewatch-probability sentence, display only. Three states, modeled on
 *  `watchReach.ts`'s three-state reads: `"measured"` names the cohort and counts, never a
 *  bare percentage, so the sentence carries its own evidence. The other two say why there
 *  is no number rather than printing one. */
function rewatchOddsSentence(odds: RewatchOdds, mediaType: string): string {
  ...
  // Season rows read as shows, the same noun the rest of the panel uses for TV: plain
  // language, no internal vocabulary. "thin" shares its catalog entry with the gate's own
  // `RewatchOddsGate` Reason (`why.rewatch_thin`): the two sentences are byte-identical, so
  // one entry serves both surfaces.
  if (odds.state === "thin") return composeReason({ k: "rewatch_thin", p: { mediaType } });
  // The Wilson upper bound the protection itself compares (gates.wilson_upper), never the
  // raw point rate: a cohort with very few of its measured titles watched again so far can
  // still read a real percentage here rather than an understated one. `bound_pct` is `null`
  // only for a row stored before it shipped; `wilsonUpperPct` recomputes the same figure
  // from `k`/`n` rather than the panel printing nothing.
...
export function WhyPanel({
  ...
  /** Open the collection screen behind this panel on the named collection. Optional so a
   *  caller with nowhere to send the jump (a test, an embedding with no queue behind it)
   *  still renders a chip that simply does nothing when pressed, never a crash. */
  ...
  // The same handoff the global Forget makes, through the same hook: that one is this
  ...
  const forgetWatchRecord = useMutation({
    ...
    onSuccess: () => {
      // Success here is the button disappearing, replaced in place by the help line below,
      // and it happens inside a `standing` notice that is correctly not a live region, so
      // nothing was said at all. This announces the same sentence as that replacement, on
      // purpose: one fact, one wording, whichever way the operator meets it.
  ...
  return (
    <WhyShell headingId={headingId} onClose={onClose}>
      ...
      {item.group_key && onShowGroup && (
        ...
        >
          {/* The glyph marks "this goes somewhere" and the destination is already in the
              words, so it is decoration sitting inside a control's accessible name. The
              space stays outside the wrapper: inside it, the name would fuse into "◂Back"
              with no pause. */}
          <span aria-hidden="true">◂</span>{" "}
      ...
      {/* The one hold the operator can lift from here, so the escape lives beside the
          sentence that names it, rather than on the Settings page, where the only control
          is the whole-library one.

          `=== true`, never truthy: the field is three-state, and `false` (the scan looked
          and the reading was honest) and `null` (a row scanned before the field existed,
          which is what the server actually sends for one) both mean this control must not
          appear. Offering to discard a watch record on a row that cannot say whether it
          needs one is the wrong direction, so "cannot tell" shows nothing.

          `standing`: this is part of the page for as long as the title is held that way,
          not a reaction to anything the operator pressed, so it is read in document order
          rather than interrupting on every render of the panel. The failure below it is
          the opposite: it answers a press, and stays an alert. */}
      {explanation.watch_blind === true && (
        <>
          <Notice tone="warn" standing as="div">
            {t("why.panel.watchBlind.notice")}
            <div className="watch-blind-act">
              {/* The confirmation fires on settled state, never at issuance. It also has to
                  exist at all. This panel is reading the scan's frozen explanation, so the
                  notice above still says "held" after the record is gone: a refetch returns
                  the same `watch_blind: true` until the next scan re-judges the row.
              ...
              {forgetWatchRecord.isSuccess ? (
                // The place to stand, and a paragraph rather than a control because the
                // press removes the only control there was, deliberately, since a second
                // press would forget a record that is already gone. The global twin hands
                // focus to a button that comes back. Here there is nothing but this
                // sentence, so it takes `tabIndex={-1}` to be able to hold focus at all.
                //
                ...
                // application rather than anywhere in this panel.
                //
      ...
      {/* This is the panel that answers "what happens to this one", so it has to say the
          ...
          allowance, because above zero that sentence would be false: the item is reapable
          then, and promising otherwise here is worse than saying nothing. The plain reason
          only, never which source was asked or when. */}
      ...
      <div className="why-actions">
        {/* When a whole-show decision keeps or reaps this season, say so above the buttons:
            the control toggles the season's own decision (override_own), and clearing a
            season key cannot clear a show-level one. Renders nothing for a movie or an
            untouched-show season. */}
        <KeptByShowNote
        ...
        <div className="why-actions-row">
          <OverrideControls
            ...
            // Same rule as the queue: reaping an item policy already condemns changes
            // nothing, so its Reap is dropped (reapIsNoop). But a spared row keeps Reap (it
            // can be flipped), and a row condemned only by its own reap keeps it (to undo).
            // Spare (rescue) always stays.
