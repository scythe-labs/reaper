 ...
 *
 *  It exists to make `gateMeta` complete: the `satisfies` clause below turns a gate added
...
export function gateMeta(mediaType: "movie" | "tv" = "movie"): Record<string, GateMeta> {
  return {
    ...
    // older scan stored.
    //
    // The entry is no longer an argument about which reader can ask for which id: `gateMeta`
