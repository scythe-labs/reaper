...
//
// Keeping an anchored popover on screen.
//
...
//
// The two popovers here, the review queue's filter menus and the policy editor's value
// suggestions, stay absolutely positioned inside their anchor, which is what keeps them glued to
// their control as the page scrolls. This slides them back into view on top of that. The account
// menu solves the same problem in CSS alone: `.user-dropdown` is `right: 0`, growing leftward
// from a corner it never leaves, and needs none of this.
//
...
// coordinates instead, clamped and flipped the same way this file's `popoverShift` slides its
// own popovers, just measured once, on open, rather than resliding on every render.
 ...
 *  snug to the trigger whatever its real rendered height is, rather than a `top` computed from
 *  a guessed one.
 *
 *  `start` is a distance from the INLINE-START edge of the viewport, for the caller to hand to
 *  `inset-inline-start`. In a left-to-right page that is the left edge and the number is a
 *  plain `left`. In a right-to-left one it is the right edge, and the popover clamps against
 *  the gutter the reader actually starts from. */
 ...
 *  long as it is open: the suggestion list refilters on each keystroke. Re-measuring is
 *  idempotent, since the inputs are the ANCHOR's position and the popover's own width, and
 *  neither moves when the popover slides, so an unchanged result bails out of the re-render
 *  instead of settling into a loop. React re-renders nothing on a window resize, hence the
 *  listener beside it. */
 ...
 *  menu grows back over the control either way.
 *
 *  `height` is only ever an upper bound for that flip decision, never a drawn coordinate. Below,
 *  the popover's top is pinned to the anchor's bottom edge. Above, its bottom is pinned just
 *  over the anchor, so it stays snug to the trigger whatever its real rendered height turns out
 *  to be: a `top` computed from a guessed height would float the popover off the button once
 *  the real content came in shorter than the guess. */
 ...
 *  collection picker's caret): open/closed state, the clamped position, and every way to
 *  dismiss it, in one hook shared by `OverrideControls.toggleMenu` and
 *  `CollectionChip.toggleOpen`.
 *
 *  `anchorRef` is the trigger the popover measures itself against on open, and treats as part
 *  of itself for the outside-click check (a click on the caret that opened the menu must
 *  toggle it, never close-then-reopen). `width`/`height` feed `fixedMenuPos`. `ignoreScrollRef`,
 *  when supplied, lets the caller suppress a scroll-close while it reads true. The Spare menu's
...
export function useFixedMenu<T extends HTMLElement = HTMLDivElement>(
  ...
  // Read fresh inside the listeners without re-subscribing them on every render.
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      // The newest layer consumes Escape, so it never also closes a panel underneath. The
      // filter popovers in ReviewQueue stop the same key for the same reason.
      e.stopPropagation();
      closeRef.current();
    };
    const onScroll = () => {
