...
// accent on the server; the whole UI derives from --accent (see styles/00-tokens.css), so
// applying it is just setting a few custom properties on <html>:
...
// The hover, soft and ring shades are pure color-mix() of --accent in the stylesheet, so they
...
/** Cache of the ink that rides on the accent, for the same reason again. index.html reads
 *  this cached value rather than recomputing `accentInk` itself, because a luminance rounded
 *  even slightly differently between the two sites can disagree near the boundary between
 *  dark and light ink (around L = 0.2055). Recomputing separately would let the pre-paint
 *  pick one ink and this module the other, flipping every accent-filled button's text color
 *  right after first paint, exactly the flash the pre-paint exists to prevent. */
 ...
 *
 *  These three mirror declarations in styles/00-tokens.css, and the AA guarantee holds only
 *  while they agree. Their twins carry a comment back to here, so a retune of either side is
 *  caught by the other:
 ...
 *
 *  `accentInk` above answers "what rides on the accent"; this answers the reverse. The
 *  built-in sky blue fails that badly in light mode (2.03:1 on white). The accent is pushed
 *  toward the theme's extreme, black on a light ground, white on a dark one, until it clears
 *  4.5:1 against the tinted ground above. Contrast rises monotonically with each step, so the
 *  first hit is the least-changed ink that reads, and an accent that already clears comes
 *  back untouched (which is what the built-in one does in dark mode).
 *
