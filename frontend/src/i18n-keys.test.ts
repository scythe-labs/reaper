...
//
// This is the missing-key gate (docs/history/I18N_PLAN.md). Every catalog key the tree
// references resolves in `locales/en/ui.json`, and every catalog message is one a reader can
// receive. This exists because translation work happens through a merge step: extraction
// agents convert files while a separate step merges their key maps into the catalog, so the
// failure this catches is precisely a key an agent used but did not return. Left uncaught,
// that would render as a raw dotted id on an operator's screen.
//
// Three things can go wrong, and each fails differently.
...
//
// Keys are read from the AST, the first argument of `t()`, `i18next.t()`, or `i18n.t()`, and
// the `i18nKey` attribute. A non-literal first argument is a runtime-composed key the gate
// cannot resolve, so composition is confined to the files listed in DYNAMIC, each claiming the
// namespaces it composes over. Those namespaces are exempt from direction 2 and are covered by
// their own suites (why.test.ts drives the composer over the real catalog).
...
// why.ts composes under any namespace (composeIn). "chip." and "warning." cover status chip
// text and policy warnings. Four one-off server sentences add three leaf keys with no id
// variant beside them ("lists.plexError", "services.modal.mapError") or with a whole small
// namespace of their own ("policySim.staleReason.", "services.discord.testResult."),
...
// the vocabulary endpoint no longer ships them. This is proven complete by
...
// errors.ts's describeError composes every coded ApiError under "error." the same way. A
// refusal's code is already the catalog's full dotted path, so the namespace prefix it strips
// before calling composeIn is proven there, not here.
// signalRamp.ts reads a built-in signal's near/far/lead/scale labels by its own signal id
// (RAMP_SPECS keyed on "unwatched", "season_rank", and so on), so `rampUnits()` hands
// i18next.t() a computed key under "signals.ramp.". This is proven complete by this module's
// own RAMP_SPECS covering every id `rampUnits` can be asked for, since a signal absent from it
// returns null rather than a partial lookup.
const DYNAMIC: Record<string, string[]> = {
  ...
  // A background job's outcome and a scan's live step are both typed reasons composed through
  // why.ts's composeIn, the same mechanism "why.ts" above already owns. These are registered
  // under the component that reads the id off the server rather than under why.ts itself,
  // since that is where each namespace's producer lives.
...
type Ref = {
  ...
  via: "t" | "trans";
  /** Trans only. The tag names its `components` prop maps, null when the prop is absent, and
   *  "opaque" when it is an expression this file-local reader cannot resolve. */
...
export function catalogRefs(fileName: string, text: string): Refs {
  ...
  // A component whose own data is named `t` destructures the translator under another name
  // (`const { t: tr } = useTranslation()`), so the callee set is built per file. It holds the
  // three canonical spellings plus every alias this file binds that way.
  ...
  // `components={shared}` spelling resolves instead of hiding the map.
  ...
  // The tag names a Trans element's `components` prop maps. This reads them off an inline
  // object literal, resolves them through a same-file binding, or falls back to "opaque" when
  // neither reading applies, since an opaque map cannot be verified, only a missing or
  // readable one can.
  ...
  const visit = (node: ts.Node): void => {
    if (ts.isCallExpression(node) && callees.has(node.expression.getText(sf))) {
      ...
      if (key === undefined) {
        // No key at all. Let it fall through as computed. Someone is doing something odd here.
        record(node, null);
...
describe("the i18n key gate", () => {
  ...
  it("every tag a message uses is one its render site can draw (#852)", () => {
    // Trans auto-renders only br, strong, i, and p (react-i18next's transKeepBasicHtmlNodesFor
    // default). Any other tag needs the `components` prop, or the operator reads the tag
    // itself, as in "Use at least 12 characters. <b>7 so far.</b>". A t() call renders text
    ...
    // The matcher's own spellings, both flagged and clean forms of both render paths.
  ...
  it("every catalog message parses as ICU", () => {
    // The parser must be able to fail, or the loop below proves nothing.
  ...
  it("reads every spelling a reference is written in (rule 147)", () => {
    ...
    // This is the alias spelling. A component whose data is named `t` binds the translator as
    ...
    // This is a .ts module. A generic call cannot swallow the t() beside it as phantom JSX.
