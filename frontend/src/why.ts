...
// Five `why.cause.*` pairs merged into one media-typed key: the movie and season versions of
// the same concept used to be two literal ids (`plex_unmatched`/`plex_season_unmatched` and
// four more like it). A scan snapshot frozen before that merge can still store the retired
// season-side id with no `mediaType` param, and the catalog no longer has an entry for it.
// This table rewrites such a row to the surviving id plus the param the merge added, so it
// renders the same "this season"/"Sonarr" wording it always did. Only rows frozen before the
// merge need this; delete it once those scans age out of the snapshots Reaper keeps
// (`services.retention.KEEP_SNAPSHOTS`).
...
// Every id that has a `mediaType` select today, but has a stored row from before that select
// existed: the five merged `cause.*` ids above, by their surviving key, plus all four reasons
// `RewatchOddsGate` emits off its cohort.
// ICU's `select` does not fall back to `other` when the variable is missing from params
// entirely; it leaves the raw template unparsed instead, proven in why.test.ts. So a row
// frozen before its select shipped would print broken syntax rather than the sentence it
// always rendered. This set defaults such a row to the wording it always had, since every
// one of them read "titles" on both lanes; a fresh row's own "movie"/"season" param
// overrides that default like any other entry the loop below sets.
...
/** The Wilson 95% upper bound of k/n, as a whole percent. Mirrors `gates.wilson_upper` (same
 *  z, same formula), with one known divergence: on an exact .5 (216 of 375 is exactly 62.5),
 *  Python's round() goes to the even neighbor where Math.round goes up, so a legacy row can
 *  read one point higher here. Display only, never a decision input. Used only to backfill
 *  `{bound_pct}` on a `rewatch_watched_again` / `rewatch_under_floor` row stored before that
 *  param shipped: a fresh row always carries its own `bound_pct` from the gate that decided
 *  it, and this never overrides one. `WhyPanel.tsx`'s own rewatch-odds display block falls
 *  back to it the same way, off `RewatchOdds.bound_pct`. */
...
/** The two `RewatchOddsGate` reasons whose sentence quotes the Wilson bound. Both always
 *  carry `k`/`n`, so a row frozen before `bound_pct` shipped can still have it backfilled
 *  from them, the same way `MEDIA_TYPED_IDS` above backfills `mediaType`. */
...
export function composeIn(namespace: string, key: ReasonKey): string {
  ...
  for (const [name, value] of Object.entries(aliased.p ?? {})) {
    if (isReasonKey(value)) {
      ...
      // through the error namespace, never `why`. Its sentence already lives at
      // `error.integration.timed_out`, the same entry the backend's own `english()` reads,
      // so this catalog never needs its own `why.error.integration.timed_out` key.
  ...
  // An id this build's catalog has no entry for: either a stored legacy sentence riding in
  // a slot, or a row written by a newer build. Render something that identifies the row
  // rather than dropping it, the same never-blank-the-panel rule the backend readers follow,
  // with the namespace prefix stripped so a legacy cause sentence reads as itself.
...
/** The card's amber-pill span, composed from a fresh row's raw day count. Null on a legacy
 *  row, when the signal was not evaluated, or when the row carries neither; the pill hides
 *  in all three cases. */
