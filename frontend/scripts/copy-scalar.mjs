...
// (GET /api/docs, see src/reaper/main.py) loads it from /vendor/scalar.js -- no CDN,
// works offline, pinned by the npm lockfile like every other dependency.
//
// Runs from the predev/prebuild hooks so it can never be stale; public/vendor is
// gitignored because it is a build product, not source.
